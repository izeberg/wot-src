package net.wg.gui.lobby.vehicleCustomization
{
   import flash.display.DisplayObject;
   import flash.display.InteractiveObject;
   import flash.events.Event;
   import flash.events.MouseEvent;
   import flash.text.TextField;
   import flash.text.TextFormat;
   import net.wg.data.constants.generated.CUSTOMIZATION_CONSTS;
   import net.wg.gui.components.controls.CheckBox;
   import net.wg.gui.components.controls.DropdownMenu;
   import net.wg.gui.components.controls.SimpleTileList;
   import net.wg.gui.components.controls.events.RendererEvent;
   import net.wg.gui.components.popovers.PopOver;
   import net.wg.gui.components.popovers.PopOverConst;
   import net.wg.gui.interfaces.ISoundButtonEx;
   import net.wg.gui.lobby.vehicleCustomization.data.FiltersPopoverVO;
   import net.wg.gui.lobby.vehicleCustomization.events.CustomizationEvent;
   import net.wg.infrastructure.base.meta.ICustomizationFiltersPopoverMeta;
   import net.wg.infrastructure.base.meta.impl.CustomizationFiltersPopoverMeta;
   import net.wg.infrastructure.managers.ITooltipMgr;
   import net.wg.infrastructure.managers.counter.CounterProps;
   import net.wg.utils.IClassFactory;
   import net.wg.utils.ICounterProps;
   import scaleform.clik.constants.DirectionMode;
   import scaleform.clik.constants.InvalidationType;
   import scaleform.clik.data.DataProvider;
   import scaleform.clik.events.ButtonEvent;
   import scaleform.clik.events.ListEvent;
   
   public class CustomizationFiltersPopover extends CustomizationFiltersPopoverMeta implements ICustomizationFiltersPopoverMeta
   {
      
      private static const PADDING:int = 9;
      
      private static const ELEMENT_PADDING:int = 12;
      
      private static const BTNDEFAULT_OFFSET:int = -12;
      
      private static const SORTING_OFFSET:int = 22;
      
      private static const DD_OFFSET:int = -5;
      
      private static const CHECKBOX_OFFSET:int = -2;
      
      private static const GROUPS_SEPARATOR_OFFSET:int = -54;
      
      private static const SEPARATOR_HEIGHT:int = 2;
      
      private static const BTN_SEPARATOR_OFFSET:int = -54;
      
      private static const INV_FOCUS_CHAIN:String = "InvFocusChain";
      
      private static const LINKAGE_TOGGLE_RENDERER:String = "ToggleHotFilterSmallRendererUI";
      
      private static const FILTERS_TILE_WIDTH:int = 47;
      
      private static const FILTERS_TILE_HEIGHT:int = 22;
      
      private static const FILTERS_VERT_GAP:int = 13;
      
      private static const FILTERS_HOR_GAP:int = 12;
      
      private static const TITLE_OFFSET:int = 1;
      
      private static const COUNTER_OFFSET:int = -4;
      
      private static const GROUPS_OFFSET:int = -3;
      
      private static const ADD_TEXT_OFFSET:int = -4;
      
      private static const FILTERS_OFFSET:int = -4;
      
      private static const AFTER_FILTERS_OFFSET:int = 17;
      
      private static const AFTER_FORM_FILTERS_OFFSET:int = 18;
      
      private static const COUNTER_DELIMITER:String = " / ";
      
      private static const COUNTER_FORMAT:TextFormat = new TextFormat(null,null,15327935);
      
      private static const COUNTER_PROPS:ICounterProps = new CounterProps(2,-2);
       
      
      public var groupsSeparator:DisplayObject = null;
      
      public var btnSeparator:DisplayObject = null;
      
      public var lblTitle:TextField = null;
      
      public var counterTf:TextField = null;
      
      public var lblGroups:TextField = null;
      
      public var lblDisplayBy:TextField = null;
      
      public var ddlGroups:DropdownMenu = null;
      
      public var ddlDisplayBy:DropdownMenu = null;
      
      public var btnDefault:ISoundButtonEx = null;
      
      public var filtersMainLabel:TextField = null;
      
      public var filtersHistoricalLabel:TextField = null;
      
      public var filtersEditableLabel:TextField = null;
      
      public var filterBtnsMain:SimpleTileList = null;
      
      public var filterBtnsHistorical:SimpleTileList = null;
      
      public var filterBtnsEditable:SimpleTileList = null;
      
      public var lblFormFilters:TextField = null;
      
      public var formBtns:SimpleTileList = null;
      
      public var lblAdditional:TextField = null;
      
      public var additionalCheckBox:CheckBox = null;
      
      private var _initData:FiltersPopoverVO = null;
      
      private var _focusChain:Vector.<InteractiveObject>;
      
      private var _dpInited:Boolean = false;
      
      private var _toolTipMgr:ITooltipMgr;
      
      public function CustomizationFiltersPopover()
      {
         this._focusChain = new Vector.<InteractiveObject>();
         this._toolTipMgr = App.toolTipMgr;
         super();
      }
      
      private static function setListParams(param1:SimpleTileList) : void
      {
         var _loc2_:IClassFactory = App.utils.classFactory;
         param1.tileWidth = FILTERS_TILE_WIDTH;
         param1.tileHeight = FILTERS_TILE_HEIGHT;
         param1.verticalGap = FILTERS_VERT_GAP;
         param1.horizontalGap = FILTERS_HOR_GAP;
         param1.directionMode = DirectionMode.HORIZONTAL;
         param1.itemRenderer = _loc2_.getClass(LINKAGE_TOGGLE_RENDERER);
      }
      
      override protected function configUI() : void
      {
         super.configUI();
         this.ddlGroups.addEventListener(ListEvent.INDEX_CHANGE,this.onDdlGroupIndexChangeHandler);
         this.ddlGroups.addEventListener(MouseEvent.MOUSE_OVER,this.onDdlGroupTypeMouseOverHandler);
         this.ddlGroups.addEventListener(MouseEvent.MOUSE_OUT,this.onDdlGroupTypeMouseOutHandler);
         this.ddlDisplayBy.addEventListener(ListEvent.INDEX_CHANGE,this.onDdlDisplayByIndexChangeHandler);
         this.btnDefault.addEventListener(ButtonEvent.CLICK,this.onBtnDefaultClickHandler);
         this.additionalCheckBox.addEventListener(Event.SELECT,this.onAdditionalCheckBoxSelectHandler);
         setListParams(this.filterBtnsMain);
         setListParams(this.filterBtnsHistorical);
         setListParams(this.filterBtnsEditable);
         setListParams(this.formBtns);
         this.formBtns.addEventListener(RendererEvent.ITEM_CLICK,this.onFormBtnsItemClickHandler);
         this.filterBtnsMain.addEventListener(RendererEvent.ITEM_CLICK,this.onFilterBtnsMainItemClickHandler);
         this.filterBtnsHistorical.addEventListener(RendererEvent.ITEM_CLICK,this.onFilterBtnsHistoricalItemClickHandler);
         this.filterBtnsEditable.addEventListener(RendererEvent.ITEM_CLICK,this.onFilterBtnsEditableItemClickHandler);
      }
      
      override protected function onDispose() : void
      {
         App.utils.counterManager.removeCounter(DisplayObject(this.btnDefault));
         this.ddlGroups.removeEventListener(ListEvent.INDEX_CHANGE,this.onDdlGroupIndexChangeHandler);
         this.ddlGroups.removeEventListener(MouseEvent.MOUSE_OVER,this.onDdlGroupTypeMouseOverHandler);
         this.ddlGroups.removeEventListener(MouseEvent.MOUSE_OUT,this.onDdlGroupTypeMouseOutHandler);
         this.ddlDisplayBy.removeEventListener(ListEvent.INDEX_CHANGE,this.onDdlDisplayByIndexChangeHandler);
         this.btnDefault.removeEventListener(ButtonEvent.CLICK,this.onBtnDefaultClickHandler);
         this.filterBtnsMain.removeEventListener(RendererEvent.ITEM_CLICK,this.onFilterBtnsMainItemClickHandler);
         this.filterBtnsHistorical.removeEventListener(RendererEvent.ITEM_CLICK,this.onFilterBtnsHistoricalItemClickHandler);
         this.filterBtnsEditable.removeEventListener(RendererEvent.ITEM_CLICK,this.onFilterBtnsEditableItemClickHandler);
         this.formBtns.removeEventListener(RendererEvent.ITEM_CLICK,this.onFormBtnsItemClickHandler);
         this.additionalCheckBox.removeEventListener(Event.SELECT,this.onAdditionalCheckBoxSelectHandler);
         this.groupsSeparator = null;
         this.btnSeparator = null;
         this.lblTitle = null;
         this.counterTf = null;
         this.filtersMainLabel = null;
         this.filtersHistoricalLabel = null;
         this.filtersEditableLabel = null;
         this.lblFormFilters = null;
         this.formBtns.dispose();
         this.formBtns = null;
         this.lblGroups = null;
         this.lblDisplayBy = null;
         this.ddlGroups.dispose();
         this.ddlGroups = null;
         this.ddlDisplayBy.dispose();
         this.ddlDisplayBy = null;
         this.btnDefault.dispose();
         this.btnDefault = null;
         this._initData.dispose();
         this._initData = null;
         this._focusChain.splice(0,this._focusChain.length);
         this._focusChain = null;
         this.filterBtnsMain.dispose();
         this.filterBtnsMain = null;
         this.filterBtnsHistorical.dispose();
         this.filterBtnsHistorical = null;
         this.filterBtnsEditable.dispose();
         this.filterBtnsHistorical = null;
         this.lblAdditional = null;
         this.additionalCheckBox.dispose();
         this.additionalCheckBox = null;
         this._toolTipMgr = null;
         super.onDispose();
      }
      
      override protected function draw() : void
      {
         var _loc1_:int = 0;
         super.draw();
         if(this._initData != null)
         {
            if(isInvalid(InvalidationType.DATA))
            {
               this.lblTitle.htmlText = this._initData.lblTitle;
               this.lblGroups.htmlText = this._initData.lblGroups;
               this.lblDisplayBy.htmlText = this._initData.lblDisplayBy;
               this.filtersMainLabel.htmlText = this._initData.filtersGroupLblMain;
               this.filtersHistoricalLabel.htmlText = this._initData.filtersGroupLblHistorical;
               this.filtersEditableLabel.htmlText = this._initData.filtersGroupLblEditable;
               this.lblFormFilters.htmlText = this._initData.formsBtnsLbl;
               this.btnDefault.label = this._initData.btnDefault;
               this.btnDefault.tooltip = this._initData.btnDefaultTooltip;
               if(!this._dpInited)
               {
                  this.ddlGroups.dataProvider = new DataProvider(App.utils.data.vectorToArray(this._initData.groupType));
                  this.ddlGroups.visible = this.ddlGroups.enabled = this._initData.groupType.length > 0;
                  this.ddlDisplayBy.dataProvider = new DataProvider(App.utils.data.vectorToArray(this._initData.displayBy));
                  this.ddlDisplayBy.visible = this.ddlDisplayBy.enabled = this._initData.displayBy.length > 0;
                  this._dpInited = true;
               }
               if(this.ddlGroups.visible)
               {
                  this.ddlGroups.selectedIndex = this._initData.groupTypeSelectedIndex;
               }
               if(this.ddlDisplayBy.visible)
               {
                  this.ddlDisplayBy.selectedIndex = this._initData.displayBySelectedIndex;
               }
               this.lblGroups.visible = this.ddlGroups.visible;
               this.lblDisplayBy.visible = this.ddlDisplayBy.visible;
               this.lblAdditional.htmlText = this._initData.lblAdditional;
               this.additionalCheckBox.label = this._initData.additionalCheckBoxData.label;
               this.additionalCheckBox.toolTip = this._initData.additionalCheckBoxData.tooltip;
               invalidateSize();
            }
            if(isInvalid(InvalidationType.SIZE))
            {
               _loc1_ = ELEMENT_PADDING;
               this.lblTitle.y = _loc1_ + TITLE_OFFSET;
               _loc1_ += this.lblTitle.height + ELEMENT_PADDING;
               this.counterTf.y = _loc1_ + COUNTER_OFFSET;
               _loc1_ += this.counterTf.height + ELEMENT_PADDING;
               this.groupsSeparator.y = _loc1_ + GROUPS_SEPARATOR_OFFSET;
               _loc1_ += SEPARATOR_HEIGHT;
               if(this.ddlGroups.visible)
               {
                  this.lblGroups.y = _loc1_ + GROUPS_OFFSET;
                  _loc1_ += this.lblGroups.height;
                  this.ddlGroups.y = _loc1_ + DD_OFFSET;
                  _loc1_ += this.ddlGroups.height;
                  _loc1_ += ELEMENT_PADDING + ADD_TEXT_OFFSET;
               }
               _loc1_ = this.layoutFilterBtnGroup(this.lblFormFilters,this.formBtns,_loc1_);
               _loc1_ = this.layoutFilterBtnGroup(this.filtersMainLabel,this.filterBtnsMain,_loc1_);
               _loc1_ = this.layoutFilterBtnGroup(this.filtersHistoricalLabel,this.filterBtnsHistorical,_loc1_);
               _loc1_ = this.layoutFilterBtnGroup(this.filtersEditableLabel,this.filterBtnsEditable,_loc1_);
               if(this.lblAdditional.visible)
               {
                  this.lblAdditional.y = _loc1_;
                  _loc1_ += this.lblAdditional.height;
                  this.additionalCheckBox.y = _loc1_ + CHECKBOX_OFFSET;
                  _loc1_ += this.additionalCheckBox.height + PADDING;
               }
               _loc1_ += ELEMENT_PADDING;
               _loc1_ += ELEMENT_PADDING;
               this.btnSeparator.y = _loc1_ + BTN_SEPARATOR_OFFSET;
               _loc1_ += SEPARATOR_HEIGHT;
               if(this.ddlDisplayBy.visible)
               {
                  this.lblDisplayBy.y = _loc1_ + GROUPS_OFFSET;
                  _loc1_ += this.lblDisplayBy.height;
                  this.ddlDisplayBy.y = _loc1_ + DD_OFFSET;
                  _loc1_ += this.ddlDisplayBy.height;
                  _loc1_ += ELEMENT_PADDING + ADD_TEXT_OFFSET;
               }
               _loc1_ += SORTING_OFFSET;
               this.btnDefault.y = _loc1_ + BTNDEFAULT_OFFSET;
               _loc1_ += this.btnDefault.height;
               _loc1_ += PADDING;
               setViewSize(this.width,_loc1_);
            }
         }
         if(isInvalid(INV_FOCUS_CHAIN))
         {
            this.refreshFocusChain();
         }
      }
      
      override protected function setData(param1:FiltersPopoverVO) : void
      {
         this._initData = param1;
         this.filterBtnsMain.dataProvider = this._initData.filterBtnsGroupMain;
         this.filterBtnsHistorical.dataProvider = this._initData.filterBtnsGroupHistorical;
         this.filterBtnsEditable.dataProvider = this._initData.filterBtnsGroupEditable;
         this.formBtns.dataProvider = this._initData.formsBtns;
         this.formBtns.visible = this._initData.formsBtns && this._initData.formsBtns.length;
         this.lblFormFilters.visible = this.formBtns.visible;
         this.additionalCheckBox.selected = this._initData.additionalCheckBoxData.selected;
         this.additionalCheckBox.visible = this.lblAdditional.visible = this._initData.additionalEnabled;
         this.filtersMainLabel.visible = this.filterBtnsEditable.visible = this._initData.filterBtnsGroupMain && this._initData.filterBtnsGroupMain.length > 0;
         this.filtersHistoricalLabel.visible = this.filterBtnsHistorical.visible = this._initData.filterBtnsGroupHistorical && this._initData.filterBtnsGroupHistorical.length > 0;
         this.filtersEditableLabel.visible = this.filterBtnsEditable.visible = this._initData.filterBtnsGroupEditable && this._initData.filterBtnsGroupEditable.length > 0;
         invalidateData();
      }
      
      override protected function initLayout() : void
      {
         popoverLayout.preferredLayout = PopOverConst.ARROW_BOTTOM;
         PopOver(wrapper).isCloseBtnVisible = true;
         super.initLayout();
      }
      
      public function as_enableDefBtn(param1:Boolean) : void
      {
         this.btnDefault.enabled = param1;
      }
      
      public function as_updateCounter(param1:int, param2:int, param3:int) : void
      {
         this.counterTf.text = VEHICLE_CUSTOMIZATION.CUSTOMIZATION_FILTERPOPOVER_COUNTER;
         var _loc4_:int = this.counterTf.text.length;
         var _loc5_:int = _loc4_ + param1.toString().length;
         this.counterTf.appendText(param1.toString() + COUNTER_DELIMITER + param2.toString());
         this.counterTf.setTextFormat(COUNTER_FORMAT,_loc4_,_loc5_);
         this.updateHiddenElementsCount(param3);
      }
      
      public function getFocusChain() : Vector.<InteractiveObject>
      {
         var _loc1_:Vector.<InteractiveObject> = new Vector.<InteractiveObject>();
         if(this.ddlGroups.visible)
         {
            _loc1_.push(this.ddlGroups);
         }
         if(this.ddlDisplayBy.visible)
         {
            _loc1_.push(this.ddlDisplayBy);
         }
         _loc1_.push(this.btnDefault,PopOver(wrapper).closeBtn);
         return _loc1_;
      }
      
      private function layoutFilterBtnGroup(param1:TextField, param2:SimpleTileList, param3:int) : int
      {
         if(param2.visible && param1.visible)
         {
            param1.y = param3 + ADD_TEXT_OFFSET;
            param3 += param1.height + FILTERS_OFFSET;
            param2.validateNow();
            param2.y = param3;
            param3 += param2.height + AFTER_FILTERS_OFFSET;
         }
         return param3;
      }
      
      private function refreshFocusChain() : void
      {
         var _loc1_:int = this._focusChain.length;
         var _loc2_:int = 0;
         while(_loc2_ < _loc1_)
         {
            this._focusChain[_loc2_].tabIndex = -1;
            _loc2_++;
         }
         this._focusChain.splice(0,this._focusChain.length);
         this._focusChain = this.getFocusChain();
         App.utils.commons.initTabIndex(this._focusChain);
         var _loc3_:InteractiveObject = null;
         if(this.ddlGroups.visible)
         {
            _loc3_ = this.ddlGroups;
         }
         if(this.ddlDisplayBy.visible)
         {
            _loc3_ = this.ddlDisplayBy;
         }
         if(_loc3_ != null)
         {
            setFocus(_loc3_);
         }
      }
      
      private function updateHiddenElementsCount(param1:int) : void
      {
         if(param1 > 0)
         {
            App.utils.counterManager.setCounter(DisplayObject(this.btnDefault),String(param1),null,COUNTER_PROPS);
         }
         else
         {
            App.utils.counterManager.removeCounter(DisplayObject(this.btnDefault));
         }
      }
      
      private function onFilterBtnsMainItemClickHandler(param1:RendererEvent) : void
      {
         onFilterChangeS(CUSTOMIZATION_CONSTS.FILTER_GROUP_MAIN,param1.index,this.filterBtnsMain.getRendererAt(param1.index).selectable);
      }
      
      private function onFilterBtnsHistoricalItemClickHandler(param1:RendererEvent) : void
      {
         onFilterChangeS(CUSTOMIZATION_CONSTS.FILTER_GROUP_HISTORICAL,param1.index,this.filterBtnsHistorical.getRendererAt(param1.index).selectable);
      }
      
      private function onFilterBtnsEditableItemClickHandler(param1:RendererEvent) : void
      {
         onFilterChangeS(CUSTOMIZATION_CONSTS.FILTER_GROUP_EDITABLE,param1.index,this.filterBtnsEditable.getRendererAt(param1.index).selectable);
      }
      
      private function onFormBtnsItemClickHandler(param1:RendererEvent) : void
      {
         onFormChangeS(param1.index,this.formBtns.getRendererAt(param1.index).selectable);
      }
      
      private function onDdlGroupIndexChangeHandler(param1:ListEvent) : void
      {
         changeGroupS(param1.index);
      }
      
      private function onDdlDisplayByIndexChangeHandler(param1:ListEvent) : void
      {
         changeDisplayMethodS(param1.index);
      }
      
      private function onBtnDefaultClickHandler(param1:ButtonEvent) : void
      {
         dispatchEvent(new CustomizationEvent(CustomizationEvent.RESET_FILTER,true));
         setDefaultFilterS();
      }
      
      private function onDdlGroupTypeMouseOverHandler(param1:MouseEvent) : void
      {
         if(!this.ddlGroups.enabled)
         {
            this._toolTipMgr.showComplex(this._initData.bonusTypeDisableTooltip);
         }
      }
      
      private function onDdlGroupTypeMouseOutHandler(param1:MouseEvent) : void
      {
         this._toolTipMgr.hide();
      }
      
      private function onAdditionalCheckBoxSelectHandler(param1:Event) : void
      {
         setHideOnAnotherVehS(this.additionalCheckBox.selected);
      }
   }
}
