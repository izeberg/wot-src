package net.wg.portal.gui.battle.battleloading
{
   ...
   import net.wg.portal.infrastructure.base.meta.IPortalBattleLoadingMeta;
   import net.wg.portal.infrastructure.base.meta.impl.PortalBattleLoadingMeta;
   import net.wg.utils.StageSizeBoundaries;
   import scaleform.clik.controls.StatusIndicator;
   
   public class PortalBattleLoading extends PortalBattleLoadingMeta implements IPortalBattleLoadingMeta
   {
      
      private static const TIPS_COUNT:uint = 2;
      ...
      private static const PROGRESS:int = InvalidationType.SYSTEM_FLAGS_BORDER << 1;
      
      {
         TIPS_SIZE[StageSizeBoundaries.WIDTH_1024] = [16,14];
         TIPS_SIZE[StageSizeBoundaries.WIDTH_1366] = [16,14];
         TIPS_SIZE[StageSizeBoundaries.WIDTH_1600] = [20,16];
         TIPS_SIZE[StageSizeBoundaries.WIDTH_1920] = [22,20];
         TIPS_SIZE[StageSizeBoundaries.WIDTH_2560] = [28,24];
         TIPS_WIDTH[StageSizeBoundaries.WIDTH_1024] = 642;
         TIPS_WIDTH[StageSizeBoundaries.WIDTH_1366] = 642;
         TIPS_WIDTH[StageSizeBoundaries.WIDTH_1600] = 791;
         TIPS_WIDTH[StageSizeBoundaries.WIDTH_1920] = 870;
         TIPS_WIDTH[StageSizeBoundaries.WIDTH_2560] = 1086;
         TIPS_BOTTOM[StageSizeBoundaries.WIDTH_1024] = [2,24];
         TIPS_BOTTOM[StageSizeBoundaries.WIDTH_1366] = [2,24];
         TIPS_BOTTOM[StageSizeBoundaries.WIDTH_1600] = [10,24];
         TIPS_BOTTOM[StageSizeBoundaries.WIDTH_1920] = [10,24];
         TIPS_BOTTOM[StageSizeBoundaries.WIDTH_2560] = [18,38];
         LOADING_BAR_WIDTH[StageSizeBoundaries.WIDTH_1024] = 412;
         LOADING_BAR_WIDTH[StageSizeBoundaries.WIDTH_1366] = 412;
         LOADING_BAR_WIDTH[StageSizeBoundaries.WIDTH_1600] = 412;
         LOADING_BAR_WIDTH[StageSizeBoundaries.WIDTH_1920] = 412;
         LOADING_BAR_WIDTH[StageSizeBoundaries.WIDTH_2560] = 412;
         ...
         LOADING_BAR_BOTTOM[StageSizeBoundaries.WIDTH_1920] = 61;
         LOADING_BAR_BOTTOM[StageSizeBoundaries.WIDTH_2560] = 61;
      }
      ...
      public function PortalBattleLoading()
      {
         super();
      }
      ...
      {
         super.configUI();
         this._tips = new <TextField>[this.tip1,this.tip2];
         this.tip1.wordWrap = this.tip2.wordWrap = true;
         this.tip1.autoSize = this.tip2.autoSize = TextFieldAutoSize.CENTER;
      }
      ...
      override protected function draw() : void
      {
         ...
         if(this._width > 0)
         {
            ...
            {
               if(this._width >= StageSizeBoundaries.WIDTH_2560 && this._height >= StageSizeBoundaries.HEIGHT_1200)
               {
                  this._state = StageSizeBoundaries.WIDTH_2560;
               }
               else if(this._width >= StageSizeBoundaries.WIDTH_1920 && this._height >= StageSizeBoundaries.HEIGHT_1080)
               {
               ...
               while(_loc6_ >= 0)
               {
                  ...
                  _loc4_.y = _loc5_ - _loc4_.height - TIPS_BOTTOM[this._state][_loc6_] | 0;
                  _loc5_ = _loc4_.y;
                  _loc6_--;
               }
            }
