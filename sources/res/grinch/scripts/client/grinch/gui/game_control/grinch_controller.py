...
from grinch.account_helpers.account_settings import isStartEventNotificationShown, isEndedEventNotificationShown, isFinishedNotificationShown, setStartEventNotificationShown, setEndedEventNotificationShown, setFinishedNotificationShown
from grinch.gui.grinch_gui_constants import PREBATTLE_ACTION_NAME, SCH_CLIENT_MSG_TYPE, FUNCTIONAL_FLAG
from grinch.gui.prebattle_hints.random_prb_hints import GrinchPrbRandomHintManager
...
class GrinchController(IGrinchController, Notifiable, SeasonProvider, IGlobalListener):
    ...
    def getAllSeasonsEndDate(self):
        seasons = list(self.getAllSeasons())
        if not seasons:
            return 0
        lastSeason = seasons[(-1)]
        if not lastSeason:
            return 0
        cycles = lastSeason.getAllCycles()
        if not cycles:
            return lastSeason.getEndDate() or 0
        lastCycle = list(cycles.values())[(-1)]
    ...
    def prbHintManager(self):
        if self.__prbHintManager is None:
            self.__prbHintManager = GrinchPrbRandomHintManager()
    ...
    def selectMode(self, useFade=False):
        ...
        if dispatcher is None:
            ...
            if self.isFrozen():
                ...
                return
            yield dispatcher.doSelectAction(PrbAction(PREBATTLE_ACTION_NAME.GRINCH), fadeCtx={'layer': WindowLayer.OVERLAY} if useFade else None)
            return
    ...
    def selectRandomMode(self, useFade=True):
        ...
        if dispatcher is None:
            ...
            yield dispatcher.doSelectAction(PrbAction(PREBATTLE_ACTION_NAME.RANDOM), fadeCtx={'layer': WindowLayer.OVERLAY} if useFade else None)
            return
    ...
    def __showNotification(self):
        ...
        if not self.isEnabled():
            return
        if periodInfo.periodType == PeriodType.AVAILABLE and not isStartEventNotificationShown():
            self.__dispatchEventStateNotification(EventStates.START, periodInfo)
            setStartEventNotificationShown(True)
        if periodInfo.periodType == PeriodType.AFTER_CYCLE and not isFinishedNotificationShown():
            self.__dispatchEventStateNotification(EventStates.BATTLES_FINISH, periodInfo)
            setFinishedNotificationShown()
        if periodInfo.periodType == PeriodType.AFTER_SEASON and not isEndedEventNotificationShown():
            self.__dispatchEventStateNotification(EventStates.ENDED, periodInfo)
            setEndedEventNotificationShown(True)
    ...
    def getTimeTillNextBattlesStart(self):
        nearestSeason = self.getNextSeason()
