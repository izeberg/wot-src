...
from grinch.gui.grinch_gui_constants import GRINCH_LOCK_SOURCE_NAME
...
_TANK_TO_ASSIST_POINTS = {VehicleTypeEnum.MEDIUMTANK: 'grinch/abilityAssistSonarPoints', 
...
_SCORING_TO_KEY = [
 ...
 (
  ScoreTypeEnum.RAM, 'grinch/rammingPoints'),
 (
...
class PostBattleView(ViewImpl):
    ...
    def refreshView(self, arenaUniqueID):
        self._battleResultsData = self.__battleResults.getResultsVO(arenaUniqueID)
        self._fillModel()
    ...
    def _onLoading(self, *args, **kwargs):
        ...
        lockNotificationManager(True, GRINCH_LOCK_SOURCE_NAME, postponeActive=True)
        Waiting.show('loadHangarSpace')
        self._fillModel()

    def _fillModel(self):
        if self._battleResultsData:
    ...
    def _onLoaded(self, *args, **kwargs):
        super(PostBattleView, self)._onLoaded(*args, **kwargs)
        lockNotificationManager(False, GRINCH_LOCK_SOURCE_NAME, releasePostponed=True)
    ...
    def _setPlayersList(self, model):
        ...
        playerSquadIndex = self._getReusableData().players.getPlayerInfo(currentAccountDBID).squadIndex
        ...
        for team in PLAYER_TEAMS:
            ...
            for vehicleData in teamVehicles:
                ...
                self._fillPlayerModel(playerModel, vehicleData, playerSquadIndex)
                if currentAccountDBID == vehicleAccountID:
                    self._fillPlayerModel(model.currentPlayer, vehicleData, playerSquadIndex)
        ...
        teamOrder.invalidate()

    def _fillPlayerModel(self, playerModel, vehicleData, playerSquadIndex):
        ...
        if playerSquadIndex == info.squadIndex:
            playerModel.setIsPlayerPlatoon(True)
    ...
    def _setBattleResult(self, model, totalProgressionPointsEarned):
        ...
        baseProgressionPoints = playerData['grinch/progressionPoints']
        totalProgressionPointsEarned += baseProgressionPoints
        ...
        model.setBattleScoreCoins(baseProgressionPoints)
    ...
    def _fillScoreList(self, model, vehicleData):
        ...
        playerScore.clear()
        playerScore.reserve(len(_SCORING_TO_KEY) + 2)
        ...
        vehicleType = VehicleTypeEnum(getVehicleClass(vehicleData['typeCompDescr']))
        if vehicleType == VehicleTypeEnum.LIGHTTANK:
            abilityAssistPoints = 0
        else:
            assistPointsKey = _TANK_TO_ASSIST_POINTS[vehicleType]
        ...
        spotPoints = vehicleData['grinch/enemyDetectionPoints']
        damageAssistPoints = vehicleData['grinch/hitAssistPoints']
        playerScore.addViewModel(self._createScoreModel(ScoreTypeEnum.SPOTANDDAMAGEASSIST, spotPoints + damageAssistPoints))
