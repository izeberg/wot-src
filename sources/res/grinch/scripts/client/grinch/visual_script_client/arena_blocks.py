...
class StoreVisualStateGO(Block, ArenaMeta):
    ...
    def __execute(self):
        ...
        if storageComponent and vehicleGO.isValid():
            import Vehicle
...
class WaitForAppearanceLoaded(Block, CGFMeta):

    def __init__(self, *args, **kwargs):
        super(WaitForAppearanceLoaded, self).__init__(*args, **kwargs)
        self._in = self._makeEventInputSlot('in', self._execute)
        self._vehicle = self._makeDataInputSlot('vehicle', SLOT_TYPE.VEHICLE)
        self._timeout = self._makeDataInputSlot('timeout', SLOT_TYPE.FLOAT)
        self._out = self._makeEventOutputSlot('out')
        self._timedOut = self._makeEventOutputSlot('timedOut')
        self._res = self._makeDataOutputSlot('res', SLOT_TYPE.VEHICLE, None)
        self.onCompositionUpdatedLookup = {}
        self.cancelHolders = {}
        return

    def _isVehicleAppearanceConstructed(self, vehicle):
        return hasattr(vehicle, 'appearance') and vehicle.isAlive() and vehicle.appearance and vehicle.appearance.isCompositionReady

    def _execute(self):
        vehicle = self._vehicle.getValue()
        if self._isVehicleAppearanceConstructed(vehicle):
            self._res.setValue(vehicle)
            self._out.call()
            return
        if not self._timeout.getValue() or not self._vehicle.getValue():
            self._timedOut.call()
            return
        self._registerCallbacks(vehicle)

    def _registerCallbacks(self, vehicle):
        onCompoundUpdatedCallback = partial(self._onCompoundReady, vehicle)
        self.onCompositionUpdatedLookup[vehicle.id] = onCompoundUpdatedCallback
        vehicle.onCompositionUpdated += onCompoundUpdatedCallback
        onCancelCallbackId = BigWorld.callback(self._timeout.getValue(), partial(self._doTimeout, vehicle))
        self.cancelHolders[vehicle.id] = onCancelCallbackId

    def _unsubscribeCallbacks(self, vehicle):
        onCompoundUpdatedCallback = self.onCompositionUpdatedLookup.pop(vehicle.id, None)
        vehicle.onCompositionUpdated -= onCompoundUpdatedCallback
        self.cancelHolders.pop(vehicle.id, None)
        return

    def _doTimeout(self, vehicle):
        self._unsubscribeCallbacks(vehicle)
        self._timedOut.call()

    def _onCompoundReady(self, vehicle, isCompoundReady):
        if isCompoundReady and self._isVehicleAppearanceConstructed(vehicle):
            self._unsubscribeCallbacks(vehicle)
            self._res.setValue(vehicle)
            self._out.call()
