...
class AdventCalendarController(IAdventCalendarController):
    ...
    def _lootBoxesTooltipConfig(self):
        if self.__lobbyContext:
            return self.__lobbyContext.getServerSettings().lootBoxesTooltipConfig
        return _LootBoxesTooltipConfig()
    ...
    def __getTimeTillPhaseChange(self):
        ...
        if self.isInActivePhase():
            return self.postEventStartDate - currentTime
        if self.isInPostActivePhase():
            startLastPostEventDay = self.postEventEndDate - time_utils.ONE_DAY
            if startLastPostEventDay > currentTime:
                return startLastPostEventDay - currentTime
            return self.postEventEndDate - currentTime
        return max(self.startDate - currentTime, 0)
