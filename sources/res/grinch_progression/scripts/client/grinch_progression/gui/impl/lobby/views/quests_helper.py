...
def sortRandomDailyQuestKey(quest):
    parts = _splitQuestID(quest.getID(), partsCount=4)
    if not parts:
        return -1
    _, number = parts[(-1)].split('_')
    if number.isdigit():
        return int(number)
    return 0
...
def isSpecialQuest(quest):
    parts = _splitQuestID(quest.getID(), 4)
    if parts:
        prefix, specialPrefix, complexity, number = parts
        return prefix.startswith(PREFIX) and specialPrefix == SPECIAL_ADD_PREFIX and complexity in Complexity.ALL and number.startswith('quest')
    return False


@dependency.replace_none_kwargs(eventsCache=IEventsCache)
def getSpecialQuestQuests(eventsCache=None):
    result = eventsCache.getAllQuests(isSpecialQuest).values()
    return sorted(result, key=sortRandomDailyQuestKey)
