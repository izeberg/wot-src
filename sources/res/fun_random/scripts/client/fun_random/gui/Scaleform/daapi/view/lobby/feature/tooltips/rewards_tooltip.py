...
_MAX_BONUS_COUNT = 8
_ALL_BONUSES_ORDER = ('lootBoxToken', 'crystal', 'premium_plus', 'freeXP', 'battlePassPoints',
                      'credits', 'items')
...
class FunRandomRewardsTooltip(AdditionalAwardTooltipData, FunProgressionWatcher):
    ...
    def _keySortOrder(cls, bonus):
        if bonus.getName() in _ALL_BONUSES_ORDER:
            return _ALL_BONUSES_ORDER.index(bonus.getName())
        return len(_ALL_BONUSES_ORDER)