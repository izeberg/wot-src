...
class FunSubModeSeasonalityConfig(namedtuple('FunSubModeSeasonalityConfig', ('isEnabled', 'peripheryIDs', 'seasons',
                                           'primeTimes', 'cycleTimes', 'geometryIDs'))):
    __slots__ = ()
...
class FunRandomConfig(namedtuple('_FunRandomConfig', ('isEnabled', 'FEPType', 'subModes', 'metaProgression',
                                'assetsPointer', 'settingsKey', 'infoPageUrl'))):
    __slots__ = ()
    ...
    def __packSubModesConfigs(cls, data):
        ...
        data['subModes'] = {int(eID):FunSubModeConfig(**eData) for eID, eData in events.iteritems() if eData.get('isEnabled', False) and eData.get('eventID', UNKNOWN_EVENT_ID) if eData.get('isEnabled', False) and eData.get('eventID', UNKNOWN_EVENT_ID)}
        return data