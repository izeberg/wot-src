...
class LootBoxesFullStatsView(ViewImpl):
    __slots__ = ('__tooltipData', '__statistic', '__lootbox', '__selectedLootBoxes',
                 '__category', '__uiLogger')
    ...
    def __isHasPreviewItems(self):
        if 'vehicles' in self.__statistic:
            return True
        if 'customizations' in self.__statistic:
            return any(item['custType'] == 'style' for item in self.__statistic['customizations'])
        return False
