...
class OneTimeGiftAdditionalRewardsReceiveFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        if not args:
            ...
            data = args[0]
            processors = (
             self.__formatCurrencies, self.__formatPremium, self.__formatItems, self.__formatVehicles,
             self.__formatGoodies, self.__formatCrewBook, self.__formatTokens, self.__formatCustomization,
    ...
    def __formatCurrencies(self, data):
        ...
        if not currencies:
            ...
            return self.__formatLines([backport.text(self.__OTG_MESSAGES.currencies())] + currencies)
    ...
    def __formatItems(self, data):
        ...
        if not itemsNames:
            ...
            return self.__formatLines([backport.text(self.__OTG_MESSAGES.equipment())] + sorted(itemsNames))
