...
from one_time_gift.gui.impl.gen.view_models.views.lobby.one_time_gift_view_model import MainViews, ModeType
...
from one_time_gift.gui.shared.processors import OneTimeGiftBranchRewardProcessor, OneTimeGiftWDRBranchRewardProcessor
...
class BranchSelectionView(SubViewBase):
    ...
    def initialize(self, branchListType, allVehiclesPurchased=False, onConfirmCallback=None, onCloseCallback=None, onErrorCallback=None):
        _logger.debug('BranchSelectionView.initialize(%s, %s, %s, %s, %s)', branchListType, allVehiclesPurchased, onConfirmCallback, onCloseCallback, onErrorCallback)
    ...
    def _getEvents(self):
        return super(BranchSelectionView, self)._getEvents() + (
         (
    ...
    def __initChildren(self):
        ...
        navigationBar = self.__childViews.get(resId)
        if navigationBar is None:
            self.__registerChild(resId, OneTimeGiftNavigationPresenter(self._onClose, self.__onInfoAction, self.__isWDRMode()))
        else:
            navigationBar.updateWDR(self.__isWDRMode())
        resId = R.aliases.lobby_header.default.Wallet()
        if resId not in self.__childViews:
            self.__registerChild(resId, WalletPresenter((
             CrystalProvider(),
             GoldProvider(),
             CreditsProvider(),
             FreeXpProvider())))
    ...
    def __requestOneTimeGift(self, branchID):
        ...
        if self.__isBranchPurchased(branch):
            ...
            return
        processor = OneTimeGiftBranchRewardProcessor if not self.__isWDRMode() else OneTimeGiftWDRBranchRewardProcessor
        self.__isConfirmationDialogBeingShown = True
        result = yield processor(branch.vehCDs, self.getParentWindow()).request()
        self.__isConfirmationDialogBeingShown = False
    ...
    def __isWDRMode(self):
        if self.parentView is not None:
            return self.parentView.getViewModel().getModeType() == ModeType.WDR
        else:
            return False