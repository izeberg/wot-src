...
from one_time_gift.gui.impl.gen.view_models.views.lobby.one_time_gift_view_model import MainViews, ModeType, OneTimeGiftViewModel
...
from one_time_gift.gui.impl.lobby.awards.wdr_compensation_view import WDRCompensationView
...
class OneTimeGiftView(ViewImpl):
    ...
    def __createSubViews(self):
        subViews = (
         IntroView(self.viewModel.introModel, self),
         ...
         WDRCompensationView(self.viewModel.wdrRewardCompensationModel, self),
    ...
    def __updateSubView(self, *args, **kwargs):
        try:
            isWDR = kwargs.pop('isWDR', False)
            with self.viewModel.transaction() as (vm):
                vm.setModeType(ModeType.WDR if isWDR else ModeType.BASIC)
...
class OneTimeGiftViewWindow(LobbyNotificationWindow):
    __slots__ = ()

    def __init__(self, viewId=MainViews.INTRO, *args, **kwargs):
        super(OneTimeGiftViewWindow, self).__init__(WindowFlags.WINDOW | WindowFlags.WINDOW_FULLSCREEN, content=OneTimeGiftView(viewId, *args, **kwargs))