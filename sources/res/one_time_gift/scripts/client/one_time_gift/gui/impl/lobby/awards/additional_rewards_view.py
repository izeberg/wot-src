...
def _getShownRewardsCount(rewardsCount, layoutTotalCount):
    if rewardsCount <= layoutTotalCount:
        return rewardsCount
    return layoutTotalCount - 1
...
class AdditionalRewardsView(BaseRewardView):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.one_time_gift.mono.lobby.one_time_gift_quest_tooltip():
            return self.__createOTGQuestTooltip(event)
        return super(AdditionalRewardsView, self).createTooltipContent(event, contentID)
    ...
    def __createOTGQuestTooltip(self, event):
        ...
        if tooltipData is None:
            ...
            return OTGQuestTooltipView(*tooltipData.specialArgs)
