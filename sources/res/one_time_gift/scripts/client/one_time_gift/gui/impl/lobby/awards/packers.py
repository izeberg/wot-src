...
class OneTimeGiftItemBonusUIPacker(ItemBonusUIPacker):
    ...
    def _getToolTip(cls, bonus):
        ...
        if equipmentSetItems:
            tooltipData.append(createTooltipData(isSpecial=True, specialArgs={'itemsForTooltip': equipmentSetItems, 'bonus': bonus}))
        for item, _ in cls.__filterBonusItems(bonus, lambda i: not isinstance(i, OptionalDevice)):
            tooltipData.append(createTooltipData(isSpecial=True, specialAlias=ItemsBonusFormatter.getTooltip(item), specialArgs=[
             item.intCD]))
