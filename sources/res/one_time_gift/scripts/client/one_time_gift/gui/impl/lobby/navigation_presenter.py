...
class OneTimeGiftNavigationPresenter(ViewComponent[NavigationBarModel], EventsHandler):
    ...
    def __init__(self, onNavigateCallback=None, onInfoActionCallback=None, isWDR=False):
    ...
    def _getEvents(self):
        ...
        events = (
         (
          model.onNavigate, partial(self.__safeCall, self.__onNavigateCallback)),
         (
          model.onInfoAction, partial(self.__safeCall, self.__onInfoActionCallback)))
        return events
    ...
    def updateWDR(self, isWDR=False):
        if isWDR != self.__isWDR:
            self.__isWDR = isWDR
            self.__update()
