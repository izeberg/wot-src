...
class BranchRewardView(BaseRewardView):
    ...
    def _setResources(self, vm):
        ...
        if self.__canSelectAnotherBranch():
    ...
    def __canSelectAnotherBranch(self):
        otgCtrl = self._oneTimeGiftController
        if otgCtrl.isCollectorsCompensationReceived():
            return False
        if otgCtrl.isPlayerNewbie() and not otgCtrl.isNewbieBranchReceived() and not otgCtrl.isBranchListPurchased(BranchListType.NEWBIE):
            return True
        if not otgCtrl.isFullListBranchReceived() and not otgCtrl.isBranchListPurchased(BranchListType.ALL):
            return True
        return False