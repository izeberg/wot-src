...
class IntroBaseState(State):
    ...
    def _onEntered(self, event):
        ...
        if machine is not None:
            ...
            otg_event_dispatcher.showIntroWindow(onConfirmCallback=lambda : machine.post(event), onCloseCallback=lambda : machine.post(event), onErrorCallback=lambda **kwargs: machine.post(StringEvent(OTGEvent.ERROR, **kwargs)), showAnimation=self.showAnimation)
        return
