...
class OneTimeGiftAccountComponent(BaseAccountExtensionComponent):
    ...
    def requestWDRBranch(self, branchToReceive, callback):
        _logger.debug('OneTimeGift: requestWDRBranch %s', branchToReceive)
        self.entity._doCmdIntArr(one_time_gift_account_commands.CMD_WDR_BRANCH, branchToReceive, lambda requestID, resultID, errorStr, ctx=None: callback(resultID, errorStr, ctx))
        return

    def requestWDRBranchCompensation(self, callback):
        _logger.debug('OneTimeGift: requestWDRBranchCompensation')
        self.entity._doCmdNoArgs(one_time_gift_account_commands.CMD_WDR_COMPENSATION, lambda requestID, resultID, errorStr, ctx=None: callback(resultID, errorStr, ctx))
        return