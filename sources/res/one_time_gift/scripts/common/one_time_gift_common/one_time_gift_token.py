...
WDR_BRANCH_BLOCKER = PREFIX + ':' + RewardType.WDR_BRANCH.value
WDR_COMPENSATION_BLOCKER = PREFIX + ':' + RewardType.WDR_COMPENSATION.value
...
def generateBranchToken(rewardType, branch, isPlayerNewbie, giftSummary):
    return (':').join([
     PREFIX, getBranchString(branch), giftSummary, rewardType.value, str(int(isPlayerNewbie))])
