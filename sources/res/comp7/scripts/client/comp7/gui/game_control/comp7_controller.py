...
class Comp7Controller(Notifiable, SeasonProvider, IComp7Controller, IGlobalListener):
        ...
        if not banners:
            ...
            for banner in banners:
                if banner['startTime'] <= currentTime <= banner['endTime']:
                    return {'state': banner['state'], 'timeLeft': banner['endTime'] - currentTime}
    ...
    def _getAlertBlockData(self):
        ...
        if self.isInPreannounceState():
            return self._ALERT_DATA_CLASS.constructForPreannounce(self.getPreannouncedSeason())
