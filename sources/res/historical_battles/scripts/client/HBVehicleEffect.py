import enum, logging, AnimationSequence, BigWorld, CGF, GenericComponents, Math, SoundGroups
...
class SequenceEffect(object):
    __slots__ = ('__active', '__animator', '__bindNode', '__spaceID', '__model', '__loadingTaskID',
                 ...
                 '__timeToFinish', '__soundGameObject', '__soundStart', '__soundEnd',
                 '__weakref__')
...
class Effect(object):
    ...
    __slots__ = ('__playerEffect', '__applyAttempts', '__owner', '__config', '__duration',
                 '__isPlaying', '__soundStart', '__soundEnd')
...
class HBVehicleEffect(DynamicScriptComponent):
    ...
    def __initSequncesData(self):
        if self.__sequences is None:
            self.__sequences = [self.__getSequence(self.OwnerType.SELF),
             self.__getSequence(self.OwnerType.TEAM_MATE),
             self.__getSequence(self.OwnerType.ENEMY)]
        return

    def __initEffectsData(self):
        if self.__effects is None:
            self.__effects = [self.__getEffect(self.OwnerType.SELF),
             self.__getEffect(self.OwnerType.TEAM_MATE),
             self.__getEffect(self.OwnerType.ENEMY)]
        return
