...
def getHBContextMenuHandlers():
    ...
    return (
     (
      BATTLE_CONTEXT_MENU_HANDLER_TYPE.HB_PLAYERS_PANEL, player_menu_handler.PlayerMenuHandler),)
...
def getHBViewSettings():
    ...
    return (
     ViewSettings(VIEW_ALIAS.HISTORICAL_BATTLE_LOADING, HistoricalBattleLoading, getSwfExtensionUrl('historical_battles', 'HBLoading.swf'), WindowLayer.TOP_WINDOW, None, ScopeTemplates.TOP_WINDOW_SCOPE),
...
def getHBBusinessHandlers():
    return (
     BattlePageBusinessHandler(VIEW_ALIAS.HISTORICAL_BATTLES), HistoricalBattlesBusinessHandler())
...
class HistoricalBattlesBusinessHandler(PackageBusinessHandler):
    ...
    def __init__(self):
        listeners = (
         (
          VIEW_ALIAS.HISTORICAL_BATTLE_LOADING, self.loadViewByCtxEvent),)
