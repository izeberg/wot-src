...
class _DamageRibbonClassFactory(_RibbonClassFactory):
    __slots__ = ('__damageCls', '__fireCls', '__ramCls', '__wcCls', '__artDmgCls',
                 '__artRocketDmgCls', '__artMortarDmgCls', '__bombDmgCls', '__bombercasDmgCls',
                 '__artFireCls', '__bombFireCls', '__recoveryCls', '__deathZoneCls',
                 '__personalDeathZoneCls', '__berserker', '__spawnedBotDmgCls', '__damageByMinefield',
                 '__damagedBySmoke', '__damagedByArtilleryOnYourself')
...
class _AssistRibbonClassFactory(_RibbonClassFactory):
    ...
    def getRibbonClass(self, event):
        if event.getBattleEventType() == BATTLE_EVENT_TYPE.TRACK_ASSIST:
            ...
            if event.getBattleEventType() == BATTLE_EVENT_TYPE.RADIO_ASSIST:
                return self.__radioAssistCls
            if event.getBattleEventType() == BATTLE_EVENT_TYPE.STUN_ASSIST:
...
def createHBRibbonsAggregator():
    if BattleReplay.g_replayCtrl.isPlaying:
        return HBRibbonsAggregatorPlayer()
    return HBRibbonsAggregator()