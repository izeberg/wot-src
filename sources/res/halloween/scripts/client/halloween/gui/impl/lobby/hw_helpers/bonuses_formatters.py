...
class HWCustomizationsBonusFormatter(CustomizationsBonusFormatter):
    ...
    def _compareFunc(self, bonusData1, bonusData2):
        ...
        return cmp((self._getPriority(self._getItemTypeName(c11nItem1)), self._getSubtypePriority(c11nItem1)), (
         self._getPriority(self._getItemTypeName(c11nItem2)), self._getSubtypePriority(c11nItem2)))

    def _getPriority(self, itemTypeName):
        if itemTypeName in self._ORDER:
            return self._ORDER.index(itemTypeName)
