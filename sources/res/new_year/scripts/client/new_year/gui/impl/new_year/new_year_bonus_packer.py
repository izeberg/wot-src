...
class NewYearTokenBonusUIPacker(TokenBonusUIPacker):
    ...
    def _getTokenBonusType(cls, tokenID, complexToken):
        ...
        if tokenID.startswith(CurrentNYConstants.HIDDEN_VEHICLE):
            return CurrentNYConstants.HIDDEN_VEHICLE
...
class NewYearStaticDogTagUIPacker(SimpleBonusUIPacker):
    ...
    def _getDogTagTooltip(cls, dogTagRecord, ctx):
        return TooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.NY_STATIC_DOG_TAG_INFO, specialArgs=[
         dogTagRecord['id'], ctx.get('topEndPos')])
