...
_FilteredToysInfo = namedtuple('_FilteredToysInfo', ('toys', 'unseenToys', 'toyCount',
                                                     'totalToyCount'))
SelectedToyFilters = namedtuple('SelectedToyFilters', ('toyTypes', 'collectionTypes',
                                                       'toyRanks'))
_ANIMATION_DELAY = 0.5
_SlotInfo = typing.NamedTuple('_SlotInfo', [('id', int)])

class NyBreakDecorationsView(HistorySubModelPresenter):
    __slots__ = ('__selectedToyFilters', '__seenToys', '__accountIsNonPlayer', '__isBreakToysProcess',
                 '__shardsTip', '__toySlots', '__selectedIndicesSlots')
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.new_year.lobby.new_year.tooltips.NyDecorationStateTooltip():
    ...
    def createPopOverContent(self, event):
        if event.contentID == R.views.new_year.lobby.new_year.popovers.NyBreakFilterPopover():
    ...
    def _getListeners(self):
        return (
         (
          NewYearEvent.ON_BREAK_TOYS_FILTER_APPLIED, self.__onFilterAppliedEvent, EVENT_BUS_SCOPE.LOBBY),)
