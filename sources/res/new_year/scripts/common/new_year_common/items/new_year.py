...
def _buildToyGroups(toysDescrs):
    ...
    toysInfos = [
     (
      currYearMaxToyRank, ToySettings.CURRENT_USUAL, TOY_USUAL_TYPES),
     (
      MAX_MEGA_TOY_RANK, ToySettings.CURRENT_MEGA, MEGA_TOY_TYPES),
     (
      MAX_PET_TOY_RANK, ToySettings.CURRENT_PET, PET_TOY_TYPES)]
...
class g_cache(object):
    ...
    def init(self, nofail=True):
        ...
        try:
            ...
            cfg['toyCountByCollectionID'] = _countToysByCollection(chain(*[ collectibles.g_cache[YEARS.getYearStrFromYearNum(year)].toys.itervalues() for year in YEARS_INFO.allYearsDecreasingIter()
                                                                          ]), collectingFunction=getToyCollectionID)
