...
def getBattleStateComponent():
    ...
    if arenaInfo:
        return arenaInfo.dynamicComponents.get('wtBattleStateComponent')
    else:
        return
...
def isEngineAuditionPresent(vehicle):
    if not vehicle or not vehicle.appearance:
        return False
    return vehicle.appearance.engineAudition is not None
