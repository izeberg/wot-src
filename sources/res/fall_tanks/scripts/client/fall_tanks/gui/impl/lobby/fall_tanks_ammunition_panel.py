...
def _getFallTanksHangarAmmoTooltipData(event):
    return createTooltipData(isSpecial=True, specialAlias=event.getArgument('slotType'), specialArgs=[
     event.getArgument('intCD')])
...
class FallTanksAmmunitionPanelView(HangarAmmunitionPanelView):
    ...
    def _getBackportTooltipData(self, event):
        if event.getArgument('slotType') in FALL_TANKS_TOOLTIPS_SET:
            return _getFallTanksHangarAmmoTooltipData(event)
        return super(FallTanksAmmunitionPanelView, self)._getBackportTooltipData(event)
...
class FallTanksHangarAmmunitionGroupsController(HangarAmmunitionGroupsController):
    ...
    def _isSwitchEnabled(self, groupSettings):
        if groupSettings.groupID == AmmunitionPanelConstants.EQUIPMENT_AND_SHELLS:
            return False
        return super(FallTanksHangarAmmunitionGroupsController, self)._isSwitchEnabled(groupSettings)
...
class FallTanksShellsBlock(ShellsBlock):
    ...
    def _getInstalled(self):
        return [
         super(FallTanksShellsBlock, self)._getInstalled()[0]]

    def _getLayout(self):
        return self._getInstalled()

    def _getKeySettings(self):
        return 'CMD_AMMO_CHOICE_1'
...
class FallTanksConsumablesBlock(ConsumablesBlock):
    ...
    def _getKeySettings(self):
        return ('CMD_AMMO_CHOICE_2', 'CMD_AMMO_CHOICE_3')
