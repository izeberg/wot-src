...
class BattleBonusesConfig(object):
    ...
    def getBonusesChoiceByType(self, bonusType, choiceID):
        ...
        if not bonuses:
            ...
            return {key:value[BattleBonusesConsts.CHOOSABLE_BONUS_NAME][choiceID] for key, value in bonuses.iteritems() if value.get(BattleBonusesConsts.CHOOSABLE_BONUS_NAME, {}).get(choiceID, None) is not None}
...
class BattleBonusApplier(object):
    ...
    def getBonusStatus(bonusType, vehType, maxReachedLevel, isRented=False):
        if bonusType == BattleBonusesConsts.XP_BONUSES:
            ...
            if vehLevel == MAX_VEHICLE_LEVEL and maxReachedLevel == MAX_ATMOSPHERE_LVL:
    ...
    def __getBonusData(cls, bonusType, choiceID, battleBonusesConfig):
        if choiceID is None:
            return battleBonusesConfig.getBonusesByType(bonusType)
        else:
            return battleBonusesConfig.getBonusesChoiceByType(bonusType, choiceID)
