...
class NYObjects(CONST_CONTAINER):
    ...
    TOWN = 'Town'
    CELEBRITY_GROUP = (
     CHALLENGE, CELEBRITY_A, CELEBRITY_CAT, CELEBRITY, CELEBRITY_D)
    UPGRADABLE_GROUP = (TREE, FIELD_KITCHEN, SCULPTURE)
    ALL = CELEBRITY_GROUP + UPGRADABLE_GROUP + (MARKETPLACE, RESOURCES, GIFT_MACHINE, GIFT_MACHINE_SIDE, TOWN)
...
class GuestsQuestsTokens(GuestsQuestsConsts):
    ...
    def getGuestCompletedTokenName(cls, guestName):
        if guestName in cls.GUESTS_ALL:
            return ('{}_quest_complete').format(guestName)
        else:
            return
    ...
    def getUIActionIconID(cls, tokenID):
        ...
        if actionType == GuestQuestTokenActionType.DECORATION:
            return _ACTION_TOKEN_DECORATION_ID_TO_UI.get(tokenID)
        return actionType
...
class NyWidgetTopMenu(object):
    ...
    ALL_PLAYER_HANGAR = (
     GLADE, FRIENDS, CHALLENGE, MARKETPLACE, GIFT_MACHINE, REWARDS)
    ALL_FRIEND_HANGAR = (FRIEND_GLADE, FRIEND_CHALLENGE)
...
class NyTabBarMainView(object):
    ...
    ALL = (
     RESOURCES, TOWN, FIR, FAIR, INSTALLATION)
...
class NyTabBarMarketplaceView(object):
    ...
    NY25_CATEGORY = Collections.NewYear25
    PREVIOUS_CATEGORY = 'previous'
    MAXIMUM_UNIQUE_TABS = 5
    ALL = (
     NY18_CATEGORY, NY19_CATEGORY, NY20_CATEGORY, NY21_CATEGORY, NY22_CATEGORY, NY23_CATEGORY,
     NY24_CATEGORY, NY25_CATEGORY)
    REVERSED_ALL = tuple(reversed(ALL))
    VISIBLE_CATEGORIES = REVERSED_ALL[:MAXIMUM_UNIQUE_TABS] + (PREVIOUS_CATEGORY,)
    PREVIOUS_CATEGORIES = REVERSED_ALL[MAXIMUM_UNIQUE_TABS:]
    REVERSED_PREVIOUS_CATEGORIES = tuple(reversed(PREVIOUS_CATEGORIES))
...
class NyTabBarChallengeView(object):
    ...
    ALL = (
     TOURNAMENT, GUEST_A, GUEST_CAT, HEADQUARTERS, GUEST_D)
...
RESOURCES_ORDER = (
 Resource.CRYSTAL, Resource.EMERALD, Resource.AMBER, Resource.IRON)
