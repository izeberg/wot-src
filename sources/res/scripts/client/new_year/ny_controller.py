...
from gui.impl.lobby.new_year.observers import HolidayOpsObjectStateObserver, HolidayOpsObserver
from gui.impl.lobby.new_year.surprise_banner import registerSurpriseBanner
...
from items.components.ny_constants import NY_STATE, TOY_TYPES_BY_OBJECT, CustomizationObjects, CurrentNYConstants, YEARS_INFO, YEARS, NY_PET_TOKEN
from items.new_year import getToyMask
from new_year.ho_surprise_banner_helper import HOSurpriseBannerHelper
from new_year.ny_constants import NewYearSysMessages, SyncDataKeys, NyWidgetTopMenu, DAYS_BETWEEN_FRIEND_TAB_REMINDER, GuestsQuestsTokens, EnvironmentState, DAY_START_TIME, DAY_END_TIME
...
class NewYearController(INewYearController, IGlobalListener):
    ...
    def __init__(self):
        ...
        self.onStateInitialized = Event(self.__em)
        ...
        self.onEnvironmentStateUpdated = Event(self.__em)
        ...
        self.__surpriseBannerHelper = HOSurpriseBannerHelper()
        self.__storedUserVehicle = None
        self.__isNyViewShown = False
        self.__isSacksMarkerShown = True
        self.__resourceTypeFrom = ''
        self.__resourceTypeTo = ''
        ...
        self.__lastViewedTab = {}
        self.__environmentState = None
        return
    ...
    def onLobbyStarted(self, ctx):
        self.__hangarSpace.setEnvironment(self.getEnvironmentState())
    ...
    def onLobbyInited(self, event):
        ...
        self.__surpriseBannerHelper.onLobbyInited()
        ...
        self.__objectObserver.onObjectStateChanged += self.__onObjectUpdate
        lsm.connect(self.__lsmObserver)
        self.__lsmObserver.onNavigationChanged += self.__onSwitchView
        self.__hangarSpace.onSpaceRefresh += self.__onSpaceRefresh
        ...
        self.onStateInitialized()
        registerSurpriseBanner()
    ...
    def checkForNewToysByType(self, objectType):
        ...
        for slot in slots:
            if self.checkForNewToysInSlot(slot.id) and slot.unlockLevelID <= self.customizationObjects.getLevel(objectType):
                return True
    ...
    @property
    def requester(self):
        if self._friendService.isInFriendHangar:
            return self.__friendRequester
    ...
    def surpriseBannerHelper(self):
        return self.__surpriseBannerHelper
    ...
    def isDogObtainTokenReceived(self):
        return self.isTokenReceived(NY_PET_TOKEN)
    ...
    def setLastViewedTab(self, menuName='', tabName='', isReset=False):
        if isReset:
            self.__lastViewedTab = {}
        if not menuName or not tabName:
            return
        self.__lastViewedTab[menuName] = tabName

    def getLastViewedTab(self, menuName):
        return self.__lastViewedTab.get(menuName, '')
    ...
    def switchEnvironmentState(self):
        currentIndex = EnvironmentState.ALL.index(self.getEnvironmentState())
        self.__environmentState = EnvironmentState.ALL[((currentIndex + 1) % len(EnvironmentState.ALL))]
        self.__hangarSpace.space.getSpace().setDefaultEnvironment(self.getEnvironmentState())
        self.__hangarSpace.space.getSpace().setEnvironment(self.getEnvironmentState())
        self.onEnvironmentStateUpdated()

    def getEnvironmentState(self):
        return self.__environmentState or self.__getDefaultEnvironmentState()

    def __getDefaultEnvironmentState(self):
        currentTime = datetime.now().hour
        if DAY_START_TIME <= currentTime < DAY_END_TIME:
    ...
    def __clear(self):
        ...
        self.__surpriseBannerHelper.clear()
        g_playerEvents.onClientUpdated -= self.__onClientUpdated
        self._eventsCache.onSyncCompleted -= self.__onEventsDataChanged
        self._friendService.onFriendHangarEnter -= self.__onFriendHangarEnter
        self._friendService.onFriendHangarExit -= self.__onFriendHangarExit
        ...
        self.__lsmObserver.onNavigationChanged -= self.__onSwitchView
        self.__hangarSpace.onSpaceRefresh -= self.__onSpaceRefresh
        self.__disconnectLSM()
        return

    def __disconnectLSM(self):
        appLoader = dependency.instance(IAppLoader)
        if appLoader.getSpaceID() != GuiGlobalSpaceID.LOBBY:
            return
        lsm = getLobbyStateMachine()
        if lsm:
    ...
    def __onObjectUpdate(self, objectName):
        isNyViewShown = objectName is not None
