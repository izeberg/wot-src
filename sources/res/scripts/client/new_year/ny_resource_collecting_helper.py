...
def isCollectingAvailable(itemsCache=None, friendService=None, forceFriend=None):
    if forceFriend is None and not friendService.isInFriendHangar or forceFriend is False:
        return getCollectingCooldownTime(itemsCache=itemsCache) == 0
    else:
        return friendService.getFriendCollectingCooldownTime() == 0
...
def getAvgResourcesByCollecting(forceFriend=None, itemsCache=None, friendService=None, forceExtraCollect=None):
    ...
    if not resources:
        ...
        return int(round(sum(resources.values()) / len(resources)))
...
class ResourceCollectingHelper(object):
    ...
    def onLobbyInited(self):
        ...
        self.__objectObserver.onObjectStateChanged += self.__onObjectStateChanged
    ...
    def getInitial3DState():
        if isCollectingAvailable():
            return 'ny_craft_state_full'
        return 'ny_craft_state_empty'
    ...
    def __getCurrentState(self):
        if isCollectingAvailable():
            return self.CollectingStates.COLLECTING_AVAILABLE
        return self.CollectingStates.DEFAULT
    ...
    def __onObjectStateChanged(self, objectName):
        self.__isResourcesCurrentObj = objectName == NYObjects.RESOURCES
