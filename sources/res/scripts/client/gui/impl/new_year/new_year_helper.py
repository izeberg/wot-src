...
_BONUSES_ORDER = ({'bonusName': 'tmanToken'}, {'bonusName': 'battleToken', 'tagStartsWith': LOOTBOX_TOKEN_PREFIX}, {'bonusName': 'entitlements'}, {'bonusName': 'crewBooks', 'tag': CREW_BOOK_RARITY.PERSONAL}, {'bonusName': 'dossier', 'tag': 'isPrefixBadge'}, {'bonusName': 'customizations', 'tag': 'style'}, {'bonusName': 'customizations'}, {'bonusName': 'dossier'}, {'bonusName': 'vehicles'}, {'bonusName': 'slots'}, {'bonusNameStartsWith': 'BlueprintNation'}, {'bonusNameStartsWith': 'Blueprint'}, {'bonusName': 'goodies'}, {'bonusName': 'crewBooks'})
_NATION_ORDER = ('ussr', 'italy', 'germany', 'usa', 'china', 'france', 'uk', 'japan',
                 'sweden', 'czech', 'poland')
...
def __getAdditionalNameBattleToken(bonus):
    ...
    if any(t.startswith(LOOTBOX_TOKEN_PREFIX) and t != coinToken for t in tokens):
        return 'lootBox'
...
def nyCreateToolTipContentDecorator(func):
    ...
    def wrapper(self, event, contentID):
        if contentID == R.views.mono.holiday_ops.tooltips.ho_discount_reward_tooltip():
