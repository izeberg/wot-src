...
class PromoCodeRewardScreenView(ViewImpl):
    ...
    def __filterRewards(rewards):
        for bonuses in rewards:
            ...
            filteredTokens = {token:val for token, val in tokens.iteritems() if not (isPromoCodeToken(token) or parseComplexToken(token).isDisplayable) if not isPromoCodeToken(token) or parseComplexToken(token).isDisplayable}
