...
_REWARD_ORDER = (
 Type.VEHICLES, Type.CUSTOMIZATIONS, Type.ATTACHMENT, Type.MODERNIZEDEQUIPMENT, Type.NYTOYS,
 Type.PREMIUMPLUS, Type.GOLD, Type.CREDITS)
...
class NyRewardKitStatistics(object):
    ...
    def getStatistics(self, lootboxID):
        ...
        if statsKey is None:
            return (rewardsData, boxesCount, None)
        else:
            return self.__itemsCache.items.tokens.getLootBoxesStats().get(statsKey, (rewardsData, boxesCount, None))
    ...
    def getRewardModel(rewardType, rewardData):
        ...
        if rewardData is None:
            ...
            if rewardType in (Type.GOLD, Type.CREDITS, Type.PREMIUMPLUS, Type.CUSTOMIZATIONS, Type.ATTACHMENT):
                count = rewardData
            else:
                if rewardType == Type.VEHICLES:
