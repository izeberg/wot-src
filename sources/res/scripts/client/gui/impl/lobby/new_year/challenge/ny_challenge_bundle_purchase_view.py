...
from gui.impl.lobby.new_year.states import GuestDState
...
_BUNDLE_TYPE_TO_CATEGORY = {NySackLootBox.LEVEL_1: BundleType.LEVEL1, NySackLootBox.LEVEL_2: BundleType.LEVEL2, 
...
def _bonusesOrderCmp(bonus1, bonus2):
    if isinstance(bonus1, CreditsBonus) and isinstance(bonus2, CreditsBonus):
        return cmp(bonus2.getValue(), bonus1.getValue())
    return cmp(_bonusesSortOrder(bonus1), _bonusesSortOrder(bonus2))
...
class BundlePurchaseDialogView(ViewImpl):
    ...
    def __init__(self, *args, **kwargs):
        settings = ViewSettings(R.views.mono.holiday_ops.dialogs.challenge.bundle_purchase_dialog())
        settings.args = args
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.mono.holiday_ops.tooltips.ho_sack_random_reward_tooltip():
            return NySackRandomRewardTooltip(event.getArgument('resourceType'))
        return super(BundlePurchaseDialogView, self).createToolTipContent(event, contentID)
    ...
    def __onBuy(self):
        ...
        if result.success:
            ...
            if bonusesData:
                pushGFNotification(GFNotificationTemplates.HO_SACKS_RARE_LOOT, bonusesData[0])
        elif result.userMsg:
        ...
        if styleItem is None:
            ...
            def _backCallback():
                if not self.__nyController.isEnabled():
                    ...
                    GuestDState.goTo(instantly=True, executeAfterLoaded=ctx)
    ...
    def __getSackState(cls, sackLevel):
        ...
        if sackLevel > currentLevel + 1:
            return SackState.LOCKED
        return SackState.AVAILABLE
