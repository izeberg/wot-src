...
class NyRewardKitMainView(LootBoxHideableView):
    ...
    __REWARD_KIT_CATEGORY_MAPPING = {NewYearCategories.NEWYEAR: ToySettings.NEW_YEAR, NewYearCategories.CHRISTMAS: ToySettings.CHRISTMAS, 
    ...
    __slots__ = ('__isVideoOff', '__tabsController', '__availableBoxes', '__isDeliveryVideoPlaying',
                 '__lastViewed', '__isWaitingToHide', '__isViewHidden', '__openBoxesFunc',
                 '__lastStatisticsResetFailed', '__rewardKitStatistics', '__isBuyViewOpen')
    ...
    def __init__(self, layoutID, lootBoxType=NewYearLootBoxes.PREMIUM, category=''):
        ...
        settings.kwargs = {'initLootBoxType': lootBoxType, 'initCategory': category}
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.lobby.new_year.tooltips.NyAttachmentsStatisticsTooltip():
            return NyAttachmentsStatisticsTooltip(self.__tabsController.getSelectedBoxID())
        return super(NyRewardKitMainView, self).createToolTipContent(event, contentID)
    ...
    def _getEvents(self):
        ...
        return events + (
         (
          model.onWindowClose, self.__onWindowClose),
         (
    ...
    def _getListeners(self):
        ...
        if not self._isMemoryRiskySystem:
            listeners += (
             (
              LootboxesEvent.ON_REWARD_VIEW_CLOSED, self.__handleRewardViewHide, EVENT_BUS_SCOPE.LOBBY),
             (
              LootboxesEvent.ON_MULTI_OPEN_VIEW_CLOSED, self.__handleRewardViewHide, EVENT_BUS_SCOPE.LOBBY))
        return listeners + (
         (
          LootboxesEvent.ON_DELIVERY_VIDEO_END, self.__onDeliveryVideoEnd, EVENT_BUS_SCOPE.LOBBY),
         (
