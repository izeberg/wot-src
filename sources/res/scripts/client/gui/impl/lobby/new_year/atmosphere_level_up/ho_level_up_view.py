import typing, AnimationSequence, BigWorld, WebBrowser
...
from gui.impl.lobby.new_year.action_helper import getButtonAction, ACTION_TO_STATES, getBonusesNames
...
from gui.impl.new_year.new_year_bonus_packer import packBonusModelAndTooltipData, getNewYearLevelUpBonusPacker, getHOLevelUpBonusSortOrder
from gui.impl.new_year.new_year_helper import ADDITIONAL_BONUS_NAME_GETTERS, backportTooltipDecorator
...
def _splitHugeBonuses(bonuses):
    ...
    if hugeBonuses:
        hugeBonuses.sort(key=getHOLevelUpBonusSortOrder)
        ...
        otherBonuses.sort(key=getHOLevelUpBonusSortOrder)
...
class HOAtmosphereLevelUpView(ViewImpl):
    ...
    def __init__(self, *args, **kwargs):
        settings = ViewSettings(R.views.mono.holiday_ops.atmosphere_level_up())
        settings.model = AtmosphereLevelUpModel()
        settings.args = args
        settings.kwargs = kwargs
        super(HOAtmosphereLevelUpView, self).__init__(settings)
        self._tooltips = {}
    ...
    @property
    def viewModel(self):
        return super(HOAtmosphereLevelUpView, self).getViewModel()
    ...
    def createToolTipContent(self, event, ctID):
        ...
        return super(HOAtmosphereLevelUpView, self).createToolTipContent(event, ctID)

    @backportTooltipDecorator()
    def createToolTip(self, event):
        return super(HOAtmosphereLevelUpView, self).createToolTip(event)
    ...
    def _getEvents(self):
        events = super(HOAtmosphereLevelUpView, self)._getEvents()
        return events + (
         (
    ...
    def _onLoading(self, *_, **kwargs):
        super(HOAtmosphereLevelUpView, self)._onLoading()
    ...
    def __onAction(self, args):
        ...
        if action is None:
            ...
            state, _ = ACTION_TO_STATES.get(actionType)
            if state:
    ...
    def __setRewards(self):
        ...
        with self.getViewModel().transaction() as (model):
            ...
            self.__fillRewardsList(rewardsList=model.hugeRewards.getItems(), bonuses=hugeBonuses, sortMethod=getHOLevelUpBonusSortOrder, packer=getNewYearLevelUpBonusPacker())
            self.__fillRewardsList(rewardsList=model.rewards.getItems(), bonuses=otherBonuses, sortMethod=getHOLevelUpBonusSortOrder, packer=getNewYearLevelUpBonusPacker())
...
class HOLevelUpWindow(LobbyNotificationWindow):
    __slots__ = ('__worldOn', '__blur')

    def __init__(self, *args, **kwargs):
        super(HOLevelUpWindow, self).__init__(content=HOAtmosphereLevelUpView(*args, **kwargs), layer=WindowLayer.OVERLAY)
    ...
    def _initialize(self):
        super(HOLevelUpWindow, self)._initialize()
    ...
    def _finalize(self):
        ...
        if self.__blur is not None:
            self.__blur.fini()
        super(HOLevelUpWindow, self)._finalize()
        return
