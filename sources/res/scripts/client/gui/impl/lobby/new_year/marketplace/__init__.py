...
_COLLECTIONS_UI_IDS_MAP = {'gift': CollectionName.GIFTSYSTEM, 
   'cat': CollectionName.CAT, 
   'atm': CollectionName.ATM, 
   'gift2': CollectionName.GIFT2, 
   'bro': CollectionName.BRO, 
   'dog': CollectionName.DOG, 
   'surprise': CollectionName.SURPRISE}
...
def getSettingsName(item):
    ...
    if collectionAction is not None:
        return collectionAction.get(MarketplaceConsts.FILL_COLLECTION_SETTING)
    else:
        _, name = item.getID().split(':')
        return _COLLECTIONS_UI_IDS_MAP.get(name, CollectionName.UNDEFINED).value
