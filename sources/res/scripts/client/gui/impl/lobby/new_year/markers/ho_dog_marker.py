...
class HODogMarker(HOHangarMarkerView):
    _nyController = dependency.descriptor(INewYearController)
    _friendService = dependency.descriptor(IFriendServiceController)

    def __init__(self, *args, **kwargs):
        settings = ViewSettings(R.views.mono.holiday_ops_marker.ho_dog_marker())
        settings.model = NyDogMarkerModel()
        settings.args = args
        settings.kwargs = kwargs
        super(HODogMarker, self).__init__(settings)
    ...
    def _onLoading(self, *args, **kwargs):
        super(HODogMarker, self)._onLoading(*args, **kwargs)
        self.__updateMarker()

    def _getEvents(self):
        events = super(HODogMarker, self)._getEvents()
        return events + (
         (
