...
_BONUSES_ORDER_GF = (
 getCoinType(), 'vehicles', NyATMReward.DOG, NyATMReward.CAT, 'tmanToken', 'customizations_style', NyATMReward.MARKETPLACE, CREW_BOOK_RARITY.PERSONAL, 'playerBadges', 'singleAchievements') + CREEBOOK_NATION_ORDER + (CREW_BOOK_RARITY.CREW_RARE, 'booster_credits', 'booster_xp', 'booster_crew_xp', CREW_BOOK_RARITY.UNIVERSAL, CREW_BOOK_RARITY.UNIVERSAL_GUIDE, 'crewBooks', 'slots', 'BlueprintUniversalFragmentCongrats') + BLUEPRINT_NATION_ORDER + ('BlueprintNationFragmentCongrats', )
...
class LevelRewardPresenter(object):
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.tooltips.AdditionalRewardsTooltip():
            ...
            if event.contentID == R.views.mono.holiday_ops.tooltips.ho_guest_tooltip():
                guestType = str(event.getArgument('guestType'))
                return NyGuestTooltip(guestType)
            if event.contentID == R.views.mono.holiday_ops.tooltips.ho_marketplace_token_tooltip():
