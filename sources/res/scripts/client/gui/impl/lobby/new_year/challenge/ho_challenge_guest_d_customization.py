...
from items.components.ny_constants import DOG_CUSTOMIZATION_SLOT_GROUP, INVALID_TOY_ID
from new_year.celebrity.celebrity_quests_helpers import getDogLevel, getTotalDogSacksCount
from new_year.newyear_cgf_components.lobby_customization_components import LobbyCustomizableObjectsManager
from new_year.ny_constants import GuestsQuestsTokens, NYObjects, SyncDataKeys
...
class HOChallengeGuestDCustomization(SubModelPresenter):
    ...
    def __init__(self, viewModel, parentView):
        super(HOChallengeGuestDCustomization, self).__init__(viewModel, parentView)
        self.__blur = None
        self.__highlightedSlotId = None
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.mono.holiday_ops.tooltips.ho_sacks_tooltip():
        ...
        if contentID == R.views.mono.holiday_ops.tooltips.ho_dog_decoration_tooltip():
            toyID = event.getArgument('toyID')
            state = event.getArgument('state')
            return NyDogDecorationTooltip(toyID, state)
        return super(HOChallengeGuestDCustomization, self).createToolTipContent(event, contentID)

    def initialize(self, *args, **kwargs):
        super(HOChallengeGuestDCustomization, self).initialize(*args, **kwargs)
    ...
    def finalize(self):
        super(HOChallengeGuestDCustomization, self).finalize()
        ...
        if self.__highlightedSlotId:
            self.__setSlotHighlight(self.__highlightedSlotId, False)
            self.__highlightedSlotId = None
        return
    ...
    def _getEvents(self):
        return super(HOChallengeGuestDCustomization, self)._getEvents() + (
         (
         ...
         (
          self.__nyController.onUpdateSlots, self.__onUpdateSlots),
         (
          self.__nyController.onSacksMarkerShow, self.__onSacksMarkerShow),
         (
          self.__nyController.sacksHelper.onSackOpened, self.__onSackOpened),
         (
          self.__friendsService.onFriendHangarEnter, self.__updateSacks),
         (
          self.__friendsService.onFriendHangarExit, self.__updateSacks),
         (
          self.__nyController.sacksHelper.onUpdated, self.__updateSacks),
         (
          self.__nyController.sacksHelper.onUpdated, self.__updateSacksEntryPoint))
    ...
    def __updateSacks(self, *args, **kwargs):
        ...
        isMarkerShown = self.__nyController.isSacksMarkerShown() and widgetState != WidgetState.ALLPURCHASED
        with self.viewModel.sacksModel.transaction() as (model):
            sacksHelper = self.__nyController.sacksHelper
            model.setMissionsCompleted(sacksHelper.getMissionsCompleted())
            model.setMissionsTotal(sacksHelper.getMissionsTotal())
            model.setMissionsCountdown(sacksHelper.getMissionsCountDown())
            model.setMissionDescription(sacksHelper.getMissionsDescription())
    ...
    def __updateSlots(self, fullUpdate, model):
        ...
        groups, groupsModel = customizationPair
    ...
    def __updateSacksEntryPoint(self):
        ...
        with self.viewModel.transaction() as (model):
            ...
            model.setHasWidgetMarker(self.__nyController.isSacksMarkerShown())

    def __updateMainState(self):
        state = ViewState.ACTIVE if self.__nyController.isTokenReceived(GuestsQuestsTokens.TOKEN_DOG) else ViewState.EMPTY
    ...
    def __onHoverSlot(self, args):
        ...
        self.__setSlotHighlight(slotId, True)
        self.__highlightedSlotId = slotId
    ...
    def __onOpenBuySacksScreen(self):
        ...
        if self.__nyController.isSacksMarkerShown():
            self.__hideInfoMarker()

    def __hideInfoMarker(self):
        self.__nyController.setIsSacksMarker(False)
        with self.viewModel.transaction() as (model):
            model.setHasWidgetMarker(False)

    def __onOpenSack(self):
        self.__nyController.sacksHelper.onOpenBox()
