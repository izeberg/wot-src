...
from gui.impl.lobby.new_year.gift_machine.ho_gift_machine_display_view import HOGiftMachineDisplayView
...
from gui.impl.lobby.new_year.states import HolidayOpsState, getMainMenuName, AssignmentsState, GladeTownState
from gui.prb_control import prbDispatcherProperty, prb_getters
from gui.shared.event_dispatcher import showGiftMachineTokenPurchaseDialog, selectVehicleInHangar, hideSquadWindow, showHangar
...
from new_year.ny_constants import NYObjects, SyncDataKeys, GuestsQuestsTokens, RESOURCES_ORDER, NyWidgetTopMenu
...
_ACTIVATE_SELECTABLE_LOGIC_STATES = (
 MachineState.IDLE,
...
class _MenuStatesObserver(BaseStateObserver):

    def __init__(self):
        super(_MenuStatesObserver, self).__init__()
        self.onSwitchView = Event.Event()

    def clear(self):
        super(_MenuStatesObserver, self).clear()
    ...
    def onEnterState(self, state, event):
        lsm = state.getMachine()
        name = getMainMenuName(state.getStateID())
        if state.getParent() == lsm.getStateByCls(HolidayOpsState):
            self.onSwitchView(name)
...
class GiftMachineCameraHelper(object):
    ...
    def getCurrentCamObjectName(self):
        ...
        if self.__cameraManager is not None:
            return self.__cameraManager.getCurrentCameraName()
        else:
            return
    ...
    def __switchCamByState(self, state):
        if NewYearNavigation.getNavigationState().isCloseMainViewInProcess:
            return
        if NewYearNavigation.getCurrentObject() not in (NYObjects.GIFT_MACHINE, NYObjects.GIFT_MACHINE_SIDE):
            return
        if state == MachineState.SPECIALREWARDPREVIEW:
...
class HOGiftMachineView(SceneRotatableView, HOSelectableLogicPresenter):
    ...
    def __init__(self, model, parent, *args, **kwargs):
        super(HOGiftMachineView, self).__init__(model, parentView=parent)
        ...
        self.__stateObserver = _MenuStatesObserver()
    ...
    def createToolTipContent(self, event, contentID):
        ...
        return super(HOGiftMachineView, self).createToolTipContent(event, contentID)

    def initialize(self, *args, **kwargs):
        super(HOGiftMachineView, self).initialize(*args, **kwargs)
    ...
    def finalize(self):
        ...
        super(HOGiftMachineView, self).finalize()
        return

    def _getEvents(self):
        events = super(HOGiftMachineView, self)._getEvents()
        unitMgr = prb_getters.getClientUnitMgr()
        if unitMgr:
            events += (
             (
              unitMgr.onUnitJoined, self.__onUnitStatusUpdated),
             (
              unitMgr.onUnitLeft, self.__onUnitStatusUpdated))
        return events + (
         (
         ...
         (
          self.__celebritySceneController.onQuestsUpdated, self.__onChallengeQuestsUpdated),
         (
          self.__wallet.onWalletStatusChanged, self.__onWalletStatusChanged),
         (
          self.__stateObserver.onSwitchView, self.__onUpdateView),
         (
    ...
    def __onUpdateView(self, menuName):
        self.__menuName = menuName
        if menuName != NyWidgetTopMenu.GIFT_MACHINE:
            return
    ...
    def __onChallengeQuestsUpdated(self):
        self.__updateConditions()
    ...
    def __onNyCoinsUpdate(self, model=None):
        ...
        if self.__isCameraHelperActive:
            ...
            count = self.__nyController.currencies.getCoinsCount()
    ...
    def __redButtonPress(self):
        self.__clearRedButtonCallback()
        self.__clearReturnToIdleCallback()
        if self.__menuName != NyWidgetTopMenu.GIFT_MACHINE:
            return
    ...
    def __tryApplyCoin(self):
        coins = self.__nyController.currencies.getCoins()
    ...
    def __isInSquad(self):
        if self.prbDispatcher is not None:
            return self.prbDispatcher.getFunctionalState().isInUnit(PREBATTLE_TYPE.SQUAD)
        else:
            return False
    ...
    def __fillRewardModel(self, state, reward):
        if reward is None:
            ...
            if state in (MachineState.REWARDPREQUEL,
             MachineState.SPECIALREWARDPREQUEL,
             MachineState.RAREREWARDPREQUEL):
    ...
    def __onGoToChallengeGuest(self):
        AssignmentsState.goTo(instantly=True)
    ...
    def __createDisplayView(self):
        if self.__displayView is None:
            self.__displayView = HOGiftMachineDisplayView()
