import wg_async as future_async, random
...
class HangarNameView(ViewImpl):
    __slots__ = ('__typeView', '__blur', '__titleId', '__descriptionId', '__isFlybyCamera')
    ...
    def _getEvents(self):
        return (
         (
          self.viewModel.onChangeAll, self.__onChangeAll),
         (
         ...
         (
          self.__nyTutorial.onFlybyFinished, self.__onFlybyFinished))

    def _onLoading(self, *args, **kwargs):
        super(HangarNameView, self)._onLoading(*args, **kwargs)
        with self.viewModel.transaction() as (tx):
            tx.setTypeView(self.__typeView)
            tx.setIsCameraFlyby(self.__isFlybyCamera)
    ...
    def __setBlur(self):
        ...
        self.__blur = CachedBlur(enabled=True, ownLayer=WindowLayer.OVERLAY - 1)

    def __onFlybyFinished(self):
        self.viewModel.setIsCameraFlyby(False)
        self.__setBlur()
        NewYearSoundsManager.playEvent(NewYearSoundEvents.CITY_NAME_SELECTING_ENTER)
...
class HangarNameViewWindow(LobbyWindow):
    ...
    def __init__(self, typeView=TypeView.WELCOME, isFlybyCamera=False):
        super(HangarNameViewWindow, self).__init__(wndFlags=WindowFlags.WINDOW | WindowFlags.WINDOW_FULLSCREEN, content=HangarNameView(R.views.mono.holiday_ops.hangar_name(), typeView, isFlybyCamera), layer=WindowLayer.OVERLAY)