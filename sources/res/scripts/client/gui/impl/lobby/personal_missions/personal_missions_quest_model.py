...
class QuestModelParser(object):
    __slots__ = ('__pm3Controller', 'generalQuestID', '__tooltipData', 'questInfo',
                 'questConfig')
    __offersProvider = dependency.descriptor(IOffersDataProvider)
    __QUEST_CONFIG_PARAMS_NAMES = ['battlesLimit', 'uniqueGoal', 'totalGoal']
    __QUEST_CONFIG_ICON_TYPE_NAMES = ['lightTank', 'mediumTank', 'heavyTank', 'AT-SPG', 'SPG']
    ...
    def __addQuestItemPartProgresses(self, progressionModel, questsConfigs, questProgress, taskName, isDone):
        ...
        if isinstance(description, PMConstants.HeaderDescription) and description.displayType == PMConstants.DISPLAY_TYPE.BIATHLON:
            ...
            if 'battlesSeries' in taskName and isinstance(description, PMConstants.HeaderDescription) and description.displayType == PMConstants.DISPLAY_TYPE.SERIES:
                ...
                return
            if not config.get('isCumulative', False) and 'damageAndAssist' not in taskName:
                return
