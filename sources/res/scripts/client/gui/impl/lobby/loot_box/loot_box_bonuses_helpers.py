...
from gui.impl.new_year.new_year_bonus_packer import NYToyBonusUIPacker
...
from gui.shared.missions.packers.bonus import BaseBonusUIPacker, getDefaultBonusPackersMap, BonusUIPacker, SimpleBonusUIPacker, VEHICLE_RENT_ICON_POSTFIX, CustomizationBonusUIPacker, ItemBonusUIPacker, BACKPORT_TOOLTIP_CONTENT_ID
...
_BONUSES_ORDER = (
 Currency.GOLD, Currency.CREDITS, Currency.CRYSTAL,
 'tankmen', PREMIUM_ENTITLEMENTS.PLUS, PREMIUM_ENTITLEMENTS.BASIC, 'freeXP', 'freeXPFactor', 'creditsFactor',
 'items', 'berths', 'dossier', 'goodies', 'tokens', 'blueprints', 'crewSkins',
 CrewBonusTypes.CREW_BOOK_BONUSES, CrewBonusTypes.CREW_SKIN_BONUSES,
 'finalBlueprints', 'attachment', 'vehicle', 'attachmentWithEffect', 'customizations',
 'vehicleWithVideo', 'vehicles', 'slots', 'modernizedEquipment', CurrentNYConstants.TOYS)
NEWYEAR_BONUS_ORDER = (
 NyCurrency.CRYSTAL, NyCurrency.EMERALD, NyCurrency.AMBER, NyCurrency.IRON) + _BONUSES_ORDER
CHRISTMAS_BONUS_ORDER = (NyCurrency.EMERALD, NyCurrency.CRYSTAL, NyCurrency.AMBER, NyCurrency.IRON) + _BONUSES_ORDER
ORIENTAL_BONUS_ORDER = (NyCurrency.AMBER, NyCurrency.CRYSTAL, NyCurrency.EMERALD, NyCurrency.IRON) + _BONUSES_ORDER
FAIRYTALE_BONUS_ORDER = (NyCurrency.IRON, NyCurrency.CRYSTAL, NyCurrency.EMERALD, NyCurrency.AMBER) + _BONUSES_ORDER
...
AUTO_OPEN_OTHER_ORDER = (
 Currency.GOLD, Currency.CREDITS, PREMIUM_ENTITLEMENTS.PLUS, PREMIUM_ENTITLEMENTS.BASIC, 'freeXP',
 Currency.EQUIP_COIN, 'customizations', CrewBonusTypes.CREW_BOOK_BONUSES, 'common', 'battle_bonus_x5',
 'booster_credits', 'booster_xp', 'booster_xp_premium', 'booster_free_xp_and_crew_xp',
 'booster_free_xp_and_crew_xp_premium', 'goodies', 'items', 'largeRepairkit', 'largeMedkit', 'autoExtinguishers',
 'smallRepairkit', 'smallMedkit', 'handExtinguishers', 'slots')
_GROUPED_SORT_C11N_INDEX = ('style_3d', 'attachment_legendary', 'attachment_epic',
                            'attachment_rare', 'style', 'projectionDecal', 'decal',
                            'emblem', 'inscription', 'insignia', 'personalNumber',
                            'paint')
_COMMON_BONUSES_ORDER = ({'getName': 'premium_plus'}, {'getName': 'gold'}, {'getName': 'credits'}, {'getName': 'slots'})
...
def compareCustomization(zipped):
    ...
    if iconName in _GROUPED_SORT_C11N_INDEX:
        return _GROUPED_SORT_C11N_INDEX.index(iconName)
    return len(_GROUPED_SORT_C11N_INDEX)
...
class NyCurrenciesBonusPacker(SimpleBonusUIPacker):
    ...
    def _getContentId(cls, bonus):
        if bonus.getCode() in NyCurrency.ALL:
            return [R.views.mono.holiday_ops.tooltips.ho_resource_tooltip()]
        return [
...
class _NYSackRandomBonusUIPacker(SimpleBonusUIPacker):
    ...
    def _getContentId(cls, bonus):
        return [
         R.views.mono.holiday_ops.tooltips.ho_sack_random_reward_tooltip()]
