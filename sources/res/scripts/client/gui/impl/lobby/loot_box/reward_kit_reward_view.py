...
class RewardKitRewardView(LootBoxHideableView, UsersInfoHelper):
    ...
    __slots__ = ('__rewards', '__boxItem', '__tooltips', '__isBackward', '__showSpecialRewardFunc',
                 '__boxID', '__lastStatisticsResetFailed', '__isGuaranteedReward',
                 '__giftsInfo')
    __stateKeeper = _StateKeeper({'_RewardKitRewardView__rewards': True, '_RewardKitRewardView__boxItem': False})
    ...
    def _getListeners(self):
        ...
        if not self._isMemoryRiskySystem:
            listeners += (
             (
              LootboxesEvent.ON_SHOW_SPECIAL_REWARDS_CLOSED, self.__onSpecialRewardsClosed, EVENT_BUS_SCOPE.LOBBY),)
        else:
            listeners += (
             (
              LootboxesEvent.ON_VIDEO_OFF_MOVIE_LOADED, self.__onVideoOffMovieLoaded, EVENT_BUS_SCOPE.LOBBY),)
        return listeners + (
         (
          CustomizationEvent.ON_RARITY_REWARD_SCREEN_CLOSED, self._onRewardScreenClosed, EVENT_BUS_SCOPE.LOBBY),
         (
