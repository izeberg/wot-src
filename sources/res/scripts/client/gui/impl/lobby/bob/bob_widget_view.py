        ...
        if tooltipId is None:
            ...
            if tooltipId == BobWidgetViewModel.PROGRESSIVE_TOOLTIP:
                ...
                if teamID is not None:
                    return createTooltipData(isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.BOB_PROGRESSIVE_INFO, specialArgs=[
                     teamID])
            elif tooltipId == BobWidgetViewModel.SKILL_TOOLTIP:
                teamID = event.getArgument('teamID')
                if teamID is not None:
                    return createTooltipData(isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.BOB_SKILL_INFO, specialArgs=[
                     teamID])
            return