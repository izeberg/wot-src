...
class CustomizationContext(object):
    ...
    def isOutfitsModified(self):
        if self.isModeChanged:
            ...
            hasDifferenceCurrentWithInitial = not any([ self.__getComponent(fullItem) for fullItem in self.mode.currentOutfit.diff(startMode.getOriginalOutfit()).itemsFull()
                                                      ]) and not any([ self.__getComponent(fullItem) for fullItem in startMode.getOriginalOutfit().diff(self.mode.currentOutfit).itemsFull()
                                                                     ])
