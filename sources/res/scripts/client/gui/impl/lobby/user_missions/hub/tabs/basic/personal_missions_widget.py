...
class PersonalMissionsWindgetPresenter(ViewComponent[PersonalMissionsModel]):
    ...
    def __getInProgressOperation(self):
        if self.__pm3Operations:
            return findFirst(lambda op: op.isInProgress(), self.__pm3Operations.values())
        else:
            return
    ...
    def __getTotalPoints(self):
        if self.__currentOperation:
            return self.__personalMissionsCache.getOperationPmPointsData(PM_BRANCH.PERSONAL_MISSION_3, self.__currentOperation.getID())
        return (0, 0)
    ...
    def __fillModel(self):
        ...
        status, nextOperationID = getDetailedOperationStatus(self.__currentOperation) if self.__currentOperation else (None,
                                                                                                                       None)
