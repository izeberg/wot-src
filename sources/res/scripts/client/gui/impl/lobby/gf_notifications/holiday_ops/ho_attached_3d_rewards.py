...
from gui.impl.lobby.gf_notifications.holiday_ops.award_notification_base import AwardNotificationBase, MAX_HUGE_REWARDS, fromRawBonusesToBonuses
...
class HOAttached3DRewards(AwardNotificationBase):
    ...
    def __init__(self, resId, *args, **kwargs):
        ...
        super(HOAttached3DRewards, self).__init__(resId, model, *args, **kwargs)
        self.__bonuses = []

    @property
    def viewModel(self):
        return super(HOAttached3DRewards, self).getViewModel()
    ...
    def _onLoading(self, *args, **kwargs):
        self.__bonuses = fromRawBonusesToBonuses(self._getPayload()['bonuses'])
        super(HOAttached3DRewards, self)._onLoading(self)
    ...
    def createToolTip(self, event):
        return super(HOAttached3DRewards, self).createToolTip(event)

    @nyCreateToolTipContentDecorator
    def createToolTipContent(self, event, contentID):
        return super(HOAttached3DRewards, self).createToolTipContent(event, contentID)

    def _getEvents(self):
        events = super(HOAttached3DRewards, self)._getEvents()
        return events + (
         (
