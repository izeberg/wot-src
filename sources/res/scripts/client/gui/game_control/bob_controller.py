...
class BobController(IBobController, Notifiable, SeasonProvider):
    ...
    def isFrozen(self):
        if findFirst(lambda primeTime: primeTime.hasAnyPeriods(), self.getPrimeTimes().values()):
            return False
        return True
    ...
    def isValidBattleType(self):
        if self.prbEntity is None:
            ...
            return bool(self.prbEntity.getModeFlags() & FUNCTIONAL_FLAG.BOB)
    ...
    def isAllZeroScore(self):
        if not self.teamsRequester.isCacheEmpty():
            return all(teamData.score == 0 for teamData in self.teamsRequester.getTeamsList())
        return False
