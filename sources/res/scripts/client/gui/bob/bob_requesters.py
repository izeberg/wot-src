import logging, weakref, typing, Event, wg_async
...
class AbstractRequester(object):
    __slots__ = ('onUpdated', '_bobController', '__cache', '__isStarted', '__subscription',
                 '_eventsManager')
    ...
    def _updateData(self, newData):
        if newData is None or not hasattr(newData, 'team'):
            return False
        if newData.team in self.__cache and cmp(self.__cache[newData.team], newData) == 0:
            return False
        else:
