...
PreformattedBonus.__new__.__defaults__ = (
 ...
 False, False, LABEL_ALIGN.CENTER, None, None, None, None, None, tuple(), False, 0)
...
class NyBattleTokeEpicBonusFormatter(SimpleBonusFormatter):
    ...
    def _format(self, bonus):
        for name in bonus.getTokens().iterkeys():
            return [
             PreformattedBonus(bonusName=name, label=self._getLabel(name), labelFormatter=self._getLabelFormatter(bonus), images=self._getImages(name), tooltip=str(R.views.lobby.new_year.tooltips.NyGuestTooltip()), align=self._getLabelAlign(bonus), isSpecial=True)]
