...
_PreformattedBonus = namedtuple('_PreformattedBonus', 'bonusName label userName images tooltip labelFormatter areTokensPawned specialArgs specialAlias isSpecial isCompensation align highlightType overlayType highlightIcon overlayIcon compensationReason postProcessTags isWulfTooltip itemTypeName newCount')
...
PreformattedBonus.__new__.__defaults__ = (
 ...
 False, False, LABEL_ALIGN.CENTER, None, None, None, None, None, tuple(), False, None, 0)
...
class CustomizationsBonusFormatter(SimpleBonusFormatter):
    ...
    def _formatBonusLabel(self, count, c11nItem=None):
        return formatCountLabel(count)
    ...
    def _createCustomizationBonus(self, bonus, item, data):
        ...
        return PreformattedBonus(bonusName=bonus.getName(), images=self._getImages(c11nItem), userName=self._getUserName(c11nItem), label=self._formatBonusLabel(item.get('value'), c11nItem), labelFormatter=self._getLabelFormatter(bonus), isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.TECH_CUSTOMIZATION_ITEM_AWARD, specialArgs=CustomizationTooltipContext(itemCD=data.get('intCD')), isCompensation=self._isCompensation(bonus), align=LABEL_ALIGN.RIGHT, itemTypeName=self._getItemTypeName(c11nItem), postProcessTags=self._makePostprocessTags(c11nItem))
...
class NyCurrenciesEpicBonusFormatter(SimpleBonusFormatter):
    ...
    def _format(self, bonus):
        return [
         PreformattedBonus(bonusName=bonus.getCode(), label=self._getLabel(bonus), labelFormatter=self._getLabelFormatter(bonus), images=self._getImages(bonus.getCode()), tooltip=str(R.views.mono.holiday_ops.tooltips.ho_resource_tooltip()), align=self._getLabelAlign(bonus), isSpecial=True)]
...
class NyBattleTokeEpicBonusFormatter(SimpleBonusFormatter):
    ...
    def _format(self, bonus):
        for name in bonus.getTokens().iterkeys():
            return [
             PreformattedBonus(bonusName=name, label=self._getLabel(name), labelFormatter=self._getLabelFormatter(bonus), images=self._getImages(name), tooltip=str(R.views.mono.holiday_ops.tooltips.ho_guest_tooltip()), align=self._getLabelAlign(bonus), isSpecial=True)]
...
class CustomizationsNYEpicBonusFormatter(CustomizationsBonusFormatter):
    ...
    @classmethod
    def _formatBonusLabel(cls, count, c11nItem=None):
...
class ItemsNYEpicBonusFormatter(ItemsEpicBonusFormatter):
    ...
    @classmethod
    def __formatHOBonusLabel(cls, item, count):
        label = ''
        if item.itemTypeID == GUI_ITEM_TYPE.BATTLE_BOOSTER:
            if count == 1:
