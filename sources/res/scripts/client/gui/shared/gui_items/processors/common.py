...
class TankmanBerthsBuyer(Processor):
    ...
    def __addPlugins(self):
        ...
        self.addPlugins([
         ...
         plugins.MessageInformator(('/').join(['buyBerthsNotEnough', self.currency]), activeHandler=lambda : not plugins.MoneyValidator(self.berthsPrice).validate().success),
         plugins.MessageConfirmator('buyBerthsConfirmation', ctx=ctx)])
