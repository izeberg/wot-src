...
class RoleSkillBattleTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, roleName):
        ...
        if equipment is None:
            ...
            items = [
             self.__packTooltipBlock(roleName, equipment, startLevel)]
            return items
...
class RoleSkillLobbyTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, equipmentName, roleName, startLevel):
        ...
        if equipment is None:
            ...
            items = filter(None, [
             self.__packHeaderBlock(equipment, roleName),
             self.__packDescriptionBlock(equipment, roleName, startLevel),
    ...
    def __packHeaderBlock(cls, equipment, roleName):
        blocks = [
         formatters.packTitleDescBlock(title=text_styles.highTitle(equipment.userString), desc=cls.__getCooldown(equipment, roleName), gap=-3),
...
class BattleResultsTrainingRatingPointsTooltip(BlocksTooltipData):
    ...
    def __packHeaderBlock(self):
        blocks = [
         formatters.packTextBlockData(text_styles.highTitle(backport.text(R.strings.comp7.battleResult.personal.tooltip.title()))),
         formatters.packTextBlockData(text_styles.main(backport.text(R.strings.comp7.training.battleResult.personal.tooltip.descr())))]
        return formatters.packBuildUpBlockData(blocks=blocks)
...
class Comp7SelectableRewardTooltip(BlocksTooltipData):
    ...
    def __packRewardsBlock():
        ...
        blocks = [
         formatters.packTextBlockData(text=text_styles.highTitle(backport.text(texts.title())), padding={'bottom': 10}),
...
def getRoleEquipmentTooltipParts(vehicle, comp7Controller=None):
    ...
    body = stripColorTagDescrTags(('\n\n').join(filter(None, (passive, active, startLevelText, cooldown))))
    return (
     roleSkill, body)
