...
class SkillTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        if skill is not None and skill.isActiveAt(time_utils.getServerUTCTime()):
            ...
            blocks = [
             self.__packActivatedSkillHeaderBlock(shortSkillName)]
            if skill.count_left > 0:
                blocks.append(self.__packCountLeftBlock(skill.count_left))
        else:
            blocks = [
             self.__packDisableSkillBlock()]
        return [
         formatters.packBuildUpBlockData(blocks=blocks, padding=formatters.packPadding(bottom=-14))]
...
class ProgressionTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        blocks = [
         ...
         self.__packProgressionTable(),
         self.__packLevelBlock(self.context.getParams().get('personalLevel', 1))]
