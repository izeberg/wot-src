...
class BasicFreeSheetTooltip(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        topItems = [
         ...
         formatters.packImageBlockData(AwardSheetPresenter.getIcon(AwardSheetPresenter.Size.BIG, branch=campaign.getID()), BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(top=-5, bottom=11))]
    ...
    def _getStatusBlock(self, operation):
        if operation.isDisabled():
            ...
            block = formatters.packBuildUpBlockData([
             ...
             formatters.packAlignedTextBlockData(text=text_styles.concatStylesToSingleLine(text_styles.main(footerDescrKey), text_styles.bonusAppliedText(currentCount), text_styles.main(' / %s' % totalCount)), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(bottom=14))])
