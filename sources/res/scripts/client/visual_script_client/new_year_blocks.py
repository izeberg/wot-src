...
from visual_script.block import EDITOR_TYPE, Block, Meta
from visual_script.slot_types import SLOT_TYPE
from visual_script.dependency import dependencyImporter
from skeletons.account_helpers.settings_core import ISettingsCore
CGF, Physics, GenericComponents, navigation, hangar_camera_manager, dependency, newYearSkeletons, nyConstants, fade_manager, adisp, ClientUpdateManager, celebrity_quests_helpers, event_dispatcher, events, guiShared, guiSharedUtils, InputHandler, gui_decorators, settings_constants, ny_gui_constants, ny_client_constants, CurrentVehicle, camera_blocks, ho_loggers = dependencyImporter('CGF', 'Physics', 'GenericComponents', 'gui.impl.new_year.navigation', 'cgf_components.hangar_camera_manager', 'helpers.dependency', 'skeletons.new_year', 'items.components.ny_constants', 'gui.Scaleform.managers.fade_manager', 'adisp', 'gui.ClientUpdateManager', 'new_year.celebrity.celebrity_quests_helpers', 'gui.shared.event_dispatcher', 'gui.shared.events', 'gui.shared', 'skeletons.gui.shared.utils', 'gui.InputHandler', 'gui.app_loader.decorators', 'account_helpers.settings_core.settings_constants', 'gui.impl.gen.view_models.views.lobby.new_year.ny_constants', 'new_year.ny_constants', 'CurrentVehicle', 'visual_script_client.camera_blocks', 'uilogging.holiday_ops.loggers')
...
class NavigateTo(Block, NewYearMeta):
    ...
    def _execute(self):
        ...
        if name == ny_client_constants.NYObjects.CHALLENGE and countOfSacks > 0 and isSelfHangar:
            name = ny_client_constants.NYObjects.CELEBRITY_D
        event_dispatcher.showHolidayOpsMainView(objectName=name, doAutoRouting=True)
        self._out.call()
        return
...
class OnNavigation(Block, NewYearMeta):
    ...
    def onStartScript(self):
        navigation.NewYearNavigation.onVSEObjectChanged += self._onObjectStateChanged

    def onFinishScript(self):
        navigation.NewYearNavigation.onVSEObjectChanged -= self._onObjectStateChanged

    def _onObjectStateChanged(self, objectName):
        self._customizationName.setValue(objectName if objectName is not None else '')
        self._out.call()
        return


class GetNavigation(Block, NewYearMeta):

    def __init__(self, *args, **kwargs):
        super(GetNavigation, self).__init__(*args, **kwargs)
        self._customizationName = self._makeDataOutputSlot('customizationName', SLOT_TYPE.STR, self._execute)
...
class OnVehicleChanged(Block, NewYearMeta):
    __hangarSpace = dependency.descriptor(guiSharedUtils.IHangarSpace)

    def __init__(self, *args, **kwargs):
        super(OnVehicleChanged, self).__init__(*args, **kwargs)
        self._out = self._makeEventOutputSlot('out')
...
class IsChallengeMainQuestsCompleted(Block, NewYearMeta):
    ...
    def _execute(self):
        self._questsCompleted.setValue(self.__celebritySceneController.isCelebrityQuestsCompleted)
...
class GetDogTokenReceived(Block, NewYearMeta):
    __nyController = dependency.descriptor(newYearSkeletons.INewYearController)

    def __init__(self, *args, **kwargs):
        super(GetDogTokenReceived, self).__init__(*args, **kwargs)
        self._isReceived = self._makeDataOutputSlot('isReceived', SLOT_TYPE.BOOL, self._exec)
...
class SendJukeboxEvent(Block, NewYearMeta):
    ...
    def validate(self):
        if not self._eventName.hasValue():
            return 'Event name value is required'
        return super(SendJukeboxEvent, self).validate()
...
class JukeboxEventHandler(Block, NewYearMeta):
    ...
    def validate(self):
        if not self._eventName.hasValue():
            return 'Event name value is required'
        return super(JukeboxEventHandler, self).validate()
...
class CreateCollider(Block, NewYearMeta):
    ...
    def _execute(self):
        if self._gameObject.hasValue() and not self._gameObject.getValue().findComponentByType(Physics.CollidersComponent):
            colliders = [
             Physics.MeshColliderDesc(self._resource.getValue(), 'default')]
...
class SwitchEnvironmentState(Block, NewYearMeta):
    __nyController = dependency.descriptor(newYearSkeletons.INewYearController)

    def __init__(self, *args, **kwargs):
        super(SwitchEnvironmentState, self).__init__(*args, **kwargs)
        self._in = self._makeEventInputSlot('in', self._execute)
        self._out = self._makeEventOutputSlot('out')
        self.__uiLogger = ho_loggers.HOBalloonLogger()

    def _execute(self):
        self.__uiLogger.logClick(self.__nyController.getEnvironmentState())
        self.__nyController.switchEnvironmentState()
        self._out.call()


class OnEnvironmentStateUpdated(Block, NewYearMeta):
    __nyController = dependency.descriptor(newYearSkeletons.INewYearController)

    def __init__(self, *args, **kwargs):
        super(OnEnvironmentStateUpdated, self).__init__(*args, **kwargs)
        self._out = self._makeEventOutputSlot('out')

    def onStartScript(self):
        self.__nyController.onEnvironmentStateUpdated += self._execute
...
class GetEnvironmentState(Block, NewYearMeta):
    __nyController = dependency.descriptor(newYearSkeletons.INewYearController)

    def __init__(self, *args, **kwargs):
        super(GetEnvironmentState, self).__init__(*args, **kwargs)
        self._state = self._makeDataOutputSlot('state', SLOT_TYPE.STR, self._execute)

    def _execute(self):
        self._state.setValue(self.__nyController.getEnvironmentState())


class NYSwitchCamera(NewYearMeta, camera_blocks.SwitchCamera):

    def _execute(self):
        if navigation.NewYearNavigation.getCurrentObject():
            super(NYSwitchCamera, self)._execute()
        else:
            self._out.call()