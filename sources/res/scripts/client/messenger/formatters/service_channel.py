...
class BattleResultsFormatter(WaitItemsSyncFormatter):
    ...
    def getCelebrityRewardsMessages(self, message):
        ...
        if completedQuestIDs is not None:
            for qID in completedQuestIDs:
                ...
                elif qID.startswith(CELEBRITY_QUESTS_PREFIX) and qID in allCelebrityQuests:
                    rawBonuses.append(message.data.get(b'detailedRewards', {}).get(qID, {}))
    ...
    def __getFairplayData(self, message):
        ...
        comp7Penalty = extraData.get(b'penalty', 0)
        comp7IsQualActive = extraData.get(b'qualActive', False)
    ...
    def getTankmenString(cls, tmen, dismiss=False):
        ...
        for tmanData in tmen:
            try:
                ...
                tmanUserStrings.append((b'{0:s} {1:s} ({2:s}, {3:s})').format(tankman.rankUserName, tankman.lastUserName, tankman.roleUserName, getUserName(tankman.vehicleNativeDescr.type)))
            except Exception:
...
class NYBreedBoughtFormatter(ServiceChannelFormatter):
    __TEMPLATE_GOLD = b'NYBreedBoughtGoldMessage'

    def format(self, message, *args):
        value = message.get(Currency.GOLD, 0)
        currency = Currency.GOLD
        template = self.__TEMPLATE_GOLD
        ctx = {b'body': backport.text(R.strings.system_messages.newYear.breed.bought.body()), 
           b'currency': backport.text(R.strings.system_messages.newYear.breed.bought.dyn(currency)(), value=value)}
        formatted = g_settings.msgTemplates.format(template, ctx)
        settings = self._getGuiSettings(message, template)
        return [
         MessageData(formatted, settings)]
...
class PrestigeFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        if not data:
            ...
            if not hasPrestige:
                return [MessageData(None, None)]
            if needShowPrestigeRewardWindow(vehCD, oldLvl, newLvl):
                return [MessageData(None, None)]
