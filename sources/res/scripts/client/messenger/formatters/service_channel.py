...
from chat_shared import SYS_MESSAGE_TYPE, MapRemovalReason, decompressSysMessage, NY_ERROR_TYPE
...
from new_year.ny_constants import TOY_COLLECTIONS, GuestQuestTokenActionType, GuestsQuestsTokens
...
class BattleResultsFormatter(WaitItemsSyncFormatter):
    ...
    def getCelebrityRewardsMessages(self, message):
        templateName = b'tokenQuests'
        messages = []
        completedQuestIDs = message.data.get(b'completedQuestIDs', set())
        allCelebrityQuests = getRewardCelebrityQuests()
        fullRewardCelebrityQuests = getRewardCelebrityQuests(fullReward=True)
        rawBonuses = []
        fullRewardRawBonuses = []
        if completedQuestIDs is not None:
            for qID in completedQuestIDs:
                if qID.startswith(CELEBRITY_QUESTS_PREFIX) and qID in allCelebrityQuests:
                    rawBonuses.append(message.data.get(b'detailedRewards', {}).get(qID, {}))
                elif qID.startswith(CELEBRITY_QUESTS_PREFIX) and qID in fullRewardCelebrityQuests:
                    fullRewardRawBonuses.append(message.data.get(b'detailedRewards', {}).get(qID, {}))

            mergedBonuses = getMergedBonusesFromDicts(rawBonuses)
            rewards = [
             NewYearChallengeAchievesFormatter.formatQuestAchieves(mergedBonuses, asBattleFormatter=False)]
            result = (b'<br/>').join(rewards) if any(rewards) else b''
            formatted = g_settings.msgTemplates.format(templateName, ctx={b'achieves': result})
            ...
            if fullRewardRawBonuses:
                messages.append(self.getCelebrityMasteryRewardMessage(fullRewardRawBonuses, message))
        return messages

    def getCelebrityMasteryRewardMessage(self, fullRewardRawBonuses, message):
        templateName = b'nyCelebrityReward'
        fullyCompletedQuestsCount = self.__celebritySceneController.fullyCompletedQuestsCount
        totalQuests = self.__celebritySceneController.questsCount
        mergedFullRewardBonuses = getMergedBonusesFromDicts(fullRewardRawBonuses)
        fullRewards = [
         NewYearChallengeAchievesFormatter.formatQuestAchieves(mergedFullRewardBonuses, asBattleFormatter=False)]
        result = (b'<br/>').join(fullRewards) if any(fullRewards) else b''
        resShortCut = R.strings.system_messages.newYear.celebrityChallenge.fullQuestReward
        if totalQuests == fullyCompletedQuestsCount:
            title = backport.text(resShortCut.final.title())
        else:
            title = backport.text(resShortCut.title(), count=fullyCompletedQuestsCount, total=totalQuests)
        formatted = g_settings.msgTemplates.format(templateName, ctx={b'title': title, b'rewards': result})
        settings = self._getGuiSettings(message, templateName, NotificationPriorityLevel.LOW)
    ...
    def _formatMessages(self, message, arenaType, arenaCreateTime):
        ...
        messages.append(MessageData(formatted, settings))
        for quest in battleResults.get(b'detailedRewards', {}).keys():
    ...
    def _prepareFormatData(self, message):
        ...
        xp += self.__getAdditionalNYBonusValue(detailedRewards, b'xp')
...
class NewYearChallengeAchievesFormatter(QuestAchievesFormatter):
    ...
    def getFormattedAchieves(cls, data, asBattleFormatter, processCustomizations=True, processTokens=True, processCompensations=True):
        result = []
        tankmanTokens = cls._processNewYearTankmanTokens(data.get(b'tokens', {}))
        if tankmanTokens:
        ...
        if itemsNames:
            result.append(cls.__makeQuestsAchieve(b'celebrityChallengeOptDevices', names=(b', ').join(itemsNames)))
        customization = data.get(b'customizations', [])
        customization.sort(key=cls.__getCustomizationOrder)
        _extendCustomizationData(data, result, htmlTplPostfix=b'QuestsReceived')
        variodicDisscounts = cls._processNewYearVariodicDisscount(data.get(b'tokens', {}))
        if variodicDisscounts:
            result.extend(variodicDisscounts)
        achievementsNames = cls._extractAchievements(data)
        if achievementsNames:
            result.append(cls.__makeQuestsAchieve(b'celebrityChallengeAchievements', achievements=(b', ').join(achievementsNames)))
        crystal = data.get(Currency.CRYSTAL, 0)
        if crystal:
            fomatter = getBWFormatter(Currency.CRYSTAL)
            result.append(cls.__makeQuestsAchieve(b'battleQuestsCrystal', crystal=fomatter(crystal)))
        ...
        if equipCoin:
            fomatter = getBWFormatter(Currency.EQUIP_COIN)
            result.append(cls.__makeQuestsAchieve(b'battleQuestsEquipCoin', equipCoin=fomatter(equipCoin)))
        coins = cls._processNewYearCoinTokens(data.get(b'tokens', {}))
        if coins:
            result.extend(coins)
        seals = cls._processNewYearSealTokens(data.get(b'tokens', {}))
        if seals:
            result.extend(seals)
        return result
    ...
    def _processNewYearCoinTokens(cls, tokens):
        ...
        if getCoinToken() not in tokens.keys():
            return []
        count = tokens[getCoinToken()][b'count']
        if count > 1:
            result.append(backport.text(R.strings.ny.systemMessage.challenge.guestQuestsCompleted.nyCoins(), count=count))
        elif count == 1:
            result.append(backport.text(R.strings.ny.systemMessage.challenge.guestQuestsCompleted.nyCoin()))
    ...
    @classmethod
    def _processNewYearSealTokens(cls, tokens):
        result = []
        if CelebrityQuestTokenParts.SEAL_TOKEN in tokens.keys():
            challengeTokenCount = tokens[CelebrityQuestTokenParts.SEAL_TOKEN][b'count']
            if challengeTokenCount > 1:
                result.append(backport.text(R.strings.ny.systemMessage.challenge.guestQuestsCompleted.ho_simple_challenge_tokens(), count=challengeTokenCount))
            elif challengeTokenCount == 1:
                result.append(backport.text(R.strings.ny.systemMessage.challenge.guestQuestsCompleted.ho_simple_challenge_token()))
        if CelebrityQuestTokenParts.FULL_SEAL_TOKEN in tokens.keys():
            masteryTokenCount = tokens[CelebrityQuestTokenParts.FULL_SEAL_TOKEN][b'count']
            if masteryTokenCount > 1:
                result.append(backport.text(R.strings.ny.systemMessage.challenge.guestQuestsCompleted.ho_mastery_challenge_tokens(), count=masteryTokenCount))
            elif masteryTokenCount == 1:
                result.append(backport.text(R.strings.ny.systemMessage.challenge.guestQuestsCompleted.ho_mastery_challenge_token()))
        return result
    ...
    def __getCustomizationOrder(cls, info):
        if info[b'custType'] in cls._CUSTOMIZATION_ORDER:
            return cls._CUSTOMIZATION_ORDER.index(info[b'custType'])
        return len(cls._CUSTOMIZATION_ORDER)
...
class NYSackBoughtFormatter(ServiceChannelFormatter):
    __TEMPLATE_CREDITS = b'NYSackBoughtCreditMessage'
    __TEMPLATE_GOLD = b'NYSackBoughtGoldMessage'

    def format(self, message, auxData):
        from new_year.celebrity.celebrity_quests_helpers import getDogLevel
        setName = backport.text(R.strings.system_messages.newYear.sack.bought.dyn((b'level{}').format(getDogLevel() + 1))())
        ...
        ctx = {b'body': backport.text(R.strings.system_messages.newYear.sack.bought.body(), setName=setName), 
           b'currency': backport.text(R.strings.system_messages.newYear.sack.bought.dyn(currency)(), count=count)}
