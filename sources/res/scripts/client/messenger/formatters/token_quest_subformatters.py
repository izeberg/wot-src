    ...
    def format(self, message, callback):
        ...
        if not isSynced:
            ...
            if self._nyController.isFinished():
                text = ('<br/>').join((
                 backport.text(R.strings.system_messages.newYear.piggyBank.rewards.text()), text))
            formatted = g_settings.msgTemplates.format(templateName, ctx={'header': title, 'text': text})
...
class ChallengeProgressionRewardFormatter(AsyncTokenQuestsSubFormatter):
    __MESSAGE_TEMPLATE = 'nyCelebrityReward'
    __QUEST_PREFIX = CelebrityQuestTokenParts.MARATHON_PREFIX
    __MARATHON_MASTERY_PREFIX = CelebrityQuestTokenParts.MARATHON_MASTERY_PREFIX
    __celebritySceneController = dependency.descriptor(ICelebritySceneController)

    def __init__(self):
        super(ChallengeProgressionRewardFormatter, self).__init__()
        self._achievesFormatter = NewYearChallengeAchievesFormatter()
    ...
    def format(self, message, callback):
        ...
        if not isSynced:
            ...
            data = message.data or {}
            completedQuestIDs = data.get('completedQuestIDs', set())
            rewardsFromQuests = getRewardsForQuests(message, completedQuestIDs)
            rewards = [
             self._achievesFormatter.formatQuestAchieves(rewardsFromQuests, asBattleFormatter=False)]
            result = ('<br/>').join(rewards) if any(rewards) else ''
            ...
            formatted = g_settings.msgTemplates.format(templateName, ctx={'title': title, 'rewards': result})
    ...
    @classmethod
    def _isQuestOfThisGroup(cls, questID):
        return questID.startswith(cls.__QUEST_PREFIX) and not questID.startswith(cls.__MARATHON_MASTERY_PREFIX)
