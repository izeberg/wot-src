...
class EventStatusBlockConstructor(StatusBlockConstructor):
    ...
    def construct(self):
        ...
        if state == VehicleStates.LOCKED.value:
            return (
             [
              formatters.packAlignedTextBlockData(text_styles.statusAlert(backport.text(R.strings.last_stand_tooltips.vehicle.rentState())), BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)],
             None, None)
        else:
            if state == VehicleStates.INBATTLE.value:
                return (
                 [
                  formatters.packAlignedTextBlockData(text_styles.critical(backport.text(R.strings.last_stand_lobby.carousel.card.inBattle())), BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)],
                 None, None)
            if state == VehicleStates.INPLATOON.value:
                return (
                 [
                  formatters.packAlignedTextBlockData(text_styles.statusAttention(backport.text(R.strings.last_stand_lobby.carousel.card.inPlatoon())), BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)],
                 None, None)
            if self.vehicle.rentalIsOver:
                if self.vehicle.isWotPlus:
                    return (
                     [
                      formatters.packAlignedTextBlockData(text_styles.critical(backport.text(R.strings.last_stand_lobby.carousel.card.suspended())), BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)],
                     None, None)
                return (
                 [
                  formatters.packAlignedTextBlockData(text_styles.critical(backport.text(R.strings.last_stand_lobby.carousel.card.rented())), BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)],
                 None, None)
            if self.vehicle.isBroken:
                return (
                 [
                  formatters.packAlignedTextBlockData(text_styles.critical(backport.text(R.strings.last_stand_lobby.carousel.card.repair())), BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)],
                 None, None)
            if not self.vehicle.isCrewFull:
                return (
                 [
                  formatters.packAlignedTextBlockData(text_styles.critical(backport.text(R.strings.last_stand_lobby.carousel.card.crewIncomplete())), BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)],
                 None, None)
            message = None
            for _, tankman in self.vehicle.crew:
                if tankman is not None:
                    if not tankman.canUseSkillsInCurrentVehicle or not tankman.isMaxRoleLevel:
                        message = (
                         [
                          formatters.packAlignedTextBlockData(text_styles.statusAlert(backport.text(R.strings.last_stand_lobby.carousel.card.untrainedCrew())), BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)],
                         None, None)
                        break
                    if tankman.currentVehicleSkillsEfficiency < MAX_SKILLS_EFFICIENCY:
                        message = (
                         [
                          formatters.packAlignedTextBlockData(text_styles.statusAlert(backport.text(R.strings.last_stand_tooltips.vehicle.lowEfficiency())), BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)],
                         None, None)
...
class EventInventoryBlockConstructor(InventoryBlockConstructor):
    ...
    def _getInstalledVehicles(self, module, inventoryVehicles):
        if 'LS_equipment' in module.tags:
            return set(v for v in inventoryVehicles.itervalues() if getLSConsumables(v).installed.containsIntCD(module.intCD))
